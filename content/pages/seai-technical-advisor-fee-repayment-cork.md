...
## 1. Cork Housing Stock Analysis: Local Challenges & Energy Deficits

Cork City and County present a distinct cross-section of Irish domestic architecture, each with specific building physics challenges under Irish Building Regulations (TGD Part L).

```
                      TYPICAL CORK HOUSING STOCK & FABRIC PROFILES
┌───────────────────────────────┬───────────────────────────────┬───────────────────────────────┐
│     Victorian / Edwardian     │    1970s–1990s Subdivisions   │       Rural Standalone        │
│   (Montenotte, St. Luke's)    │ (Douglas, Ballincollig, Carrigaline)│   (West Cork, Mallow, Midleton) │
├───────────────────────────────┼───────────────────────────────┼───────────────────────────────┤
│ • Solid sandstone/limestone   │ • 100mm uninsulated/aerated   │ • Hollow block / mass concrete│
│ • Breathable lime mortars     │   cavity block walls          │ • High wind-driven rain load  │
│ • High draft infiltration     │ • Mineral wool attic settling │ • High dependence on kerosene │
│ • Target: IWI + Breathability │ • Target: Cavity pump + Attic │ • Target: EWI + Full NZEB     │
└───────────────────────────────┴───────────────────────────────┴───────────────────────────────┘
```
...
Before installing a Heat Pump under the SEAI Better Energy Homes scheme, an **independent SEAI Technical Assessment is mandatory**. This ensures your home’s building fabric is sufficiently insulated and airtight to support low-temperature hydronic heating efficiently.

```
                   TECHNICAL ADVISOR FINANCIAL FLOW
┌────────────────────────┐      €350 Upfront      ┌────────────────────────┐
│   Cork Property Owner  │ ─────────────────────> │ Registered Advisor     │
│   (Book Assessment)    │                        │ (Completes HLI Survey) │
└────────────────────────┘                        └────────────────────────┘
            │                                                  │
            │ Apply via SEAI Portal                            │ Publishes Pre-BER
            ▼                                                  ▼ & HLI XML
┌────────────────────────┐      €350 Repayment    ┌────────────────────────┐
│  SEAI Grant Approval   │ ◄───────────────────── │ Works Completed        │
│  & Heat Pump Install   │   (Paid with HP Grant) │ & Post-BER Uploaded    │
└────────────────────────┘                        └────────────────────────┘
```

### The €350 Technical Assessment Repayment Mechanism

1. **Upfront Payment:** You engage an SEAI-registered Technical Advisor to carry out a comprehensive Heat Loss Indicator (HLI) survey and publish a Pre-Works Building Energy Rating (BER) with an XML energy report. You pay the advisor directly for this independent service.
2. **Grant Lock-in:** The Advisor submits the formal Technical Assessment to the SEAI database.
3. **Repayment Release:** When you proceed with and complete the installation of an SEAI-supported Heat Pump system, the **€350 Technical Assessment grant is credited directly back to you** alongside your primary heat pump capital grant.
...
## 3. The Path to A2 & NZEB Compliance (TGD Part L)

To qualify for heat pump integration and ensure low running costs, your home must achieve a **Heat Loss Indicator (HLI) of $\le 2.0\,\text{W/K/m}^2$** (or $\le 2.3\,\text{W/K/m}^2$ where specific technical justifications are documented).

```
                      FABRIC FIRST RETROFIT SEQUENCE
┌──────────────────────┐     ┌──────────────────────┐     ┌──────────────────────┐
│ 1. Fabric Upgrades   │ ──> │ 2. Airtightness & VM │ ──> │ 3. Heat Pump & PV    │
│ Reduce raw losses    │     │ Controlled air exch. │     │ Low temp generation  │
└──────────────────────┘     └──────────────────────┘     └──────────────────────┘
 • Wall U ≤ 0.18–0.21        • Target ≤ 3.0 m³/(h·m²)     • Flow temp ≤ 35°C–45°C
 • Roof U ≤ 0.16             • Demand Controlled Mech.    • SCOP ≥ 4.2
 • Glazing U ≤ 1.2           • Maintain IAQ               • Min 2–4 kWp Solar PV
```
...
## 4. The Homeowner Roadmap: From Survey to Payback

```
1. PRE-ASSESSMENT            2. DESIGN SPECIFICATION       3. CAPITAL WORKS
   • Book Advisor               • Fabric uplift plan          • Contractor selection
   • DEAP 4.2 Survey            • HLI reduction model         • Install insulation/HP
   • Publish Pre-BER            • SEAI Grant App lodged       • SEAI interim QA
         │                             │                             │
         └─────────────────────────────┼─────────────────────────────┘
                                       ▼
                             4. POST-BER & SIGN-OFF
                                • Final Blower-door test
                                • Publish Final A-Rating
                                • Technical Fee + Grant Drawdown
```

1. **Initial Advisor Engagement:** Book your on-site assessment. The advisor inspects building elements, verifies heating loops, measures thermal boundaries, and logs mechanical systems.
2. **Technical Modelling (DEAP 4.2):** The advisor calculates the HLI. If the HLI is $> 2.0\,\text{W/K/m}^2$, a tailored roadmap outlines the exact fabric upgrades required before a heat pump can be certified.
3. **Grant Application Submission:** Submit your application on the SEAI portal using your Advisor's ID, registered MPRN, and home details.
4. **Execution of Works:** SEAI-registered contractors complete insulation, airtightness, and heat pump installations according to SR:54 (Code of Practice for Domestic Retrofit).
5. **Post-Works Sign-Off:** The advisor conducts the post-retrofit audit, runs the final Building Energy Rating calculation, uploads the A2/A3 certificate, and triggers your **Heat Pump Grant along with the €350 Technical Assessment fee repayment**.
...
## Book Your SEAI Technical Assessment in Cork
...
<div class="ecosmart-grant-calculator my-8 p-6 bg-slate-900 text-white rounded-xl border border-emerald-500/30 shadow-2xl" data-location="Cork">
  <div class="flex items-center justify-between mb-4 border-b border-slate-800 pb-3">
    <div>
      <h3 class="text-xl font-bold text-emerald-400">SEAI Grant & Co-Funding Calculator (Cork)</h3>
...
<div class="ecosmart-stripe-bridge my-8 p-6 bg-gradient-to-br from-slate-900 via-indigo-950/40 to-slate-900 border-2 border-indigo-500/40 rounded-xl text-white shadow-2xl" data-keyword="seai technical advisor fee repayment Cork">
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
      ...
      <h3 class="text-xl font-bold text-white">Book Your Cork Home Architectural & Technical Survey</h3>
