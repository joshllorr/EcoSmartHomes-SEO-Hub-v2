...
import {
  matchContractor,
  SAMPLE_CONTRACTORS,
} from '../logic/contractors/matchContractor';
import {
  generateGrantSubmissionPayload,
  updateSubmissionLifecycleStatus,
} from '../logic/grants/submitEngine';
import {
  generatePostInstallRecord,
  updatePostInstallTimeline,
  cronSyncPostInstallRecord,
} from '../logic/postinstall/trackerEngine';
import {
...
import {
  ...
  getContractorScore,
  updateContractorScoreFromJourney,
  ContractorMetrics,
} from '../logic/contractors/contractorScoresEngine';
import {
  generateHomeUpgradeBundle,
  getHomeUpgradeBundle,
} from '../logic/upgrades/homeUpgradeEngine';
import {
  generateNationalInsights,
  getNationalInsights,
} from '../logic/insights/nationalInsightsEngine';
import {
  generateAndStoreForecast,
  getForecast,
} from '../logic/forecasting/retrofitForecastEngine';
import { generateAdvisorReply } from '../logic/advisor/retrofitAdvisorEngine';
import {
  calculateHomeownerSentiment,
  getHomeownerSentiment,
} from '../logic/sentiment/homeownerSentimentEngine';
import {
...
} from '../logic/orchestrator/masterOrchestrator';
...
async function addTimelineEvent(
  ...
  try {
    ...
    if (triggerEvents.includes(event)) {
      ...
      await updateContractorScoreFromJourney(
        env,
        defaultContractorId,
        userId,
      ).catch(() => {});
...
function detectConflict(negotiation: any, biases: Record<string, number>) {
  ...
  const conflict = approvals > 0 && rejections > 0 && biasSpread > 0.2;
...
function resolveConflict(negotiation: any, biases: Record<string, number>) {
  ...
  for (const vote of negotiation?.votes || []) {
    if (!vote.approve && (vote.notes || '').toLowerCase().includes('risk')) {
      adjustedBiases['adjust-budget'] = Number(
        ((adjustedBiases['adjust-budget'] ?? 0.5) - 0.1).toFixed(2),
      );
    }
    if (!vote.approve && (vote.notes || '').toLowerCase().includes('stress')) {
      adjustedBiases['adjust-bidding'] = Number(
        ((adjustedBiases['adjust-bidding'] ?? 0.5) - 0.1).toFixed(2),
      );
    }
  }

  adjustedBiases['adjust-keywords'] = Number(
    ((adjustedBiases['adjust-keywords'] ?? 0.5) + 0.05).toFixed(2),
  );
  adjustedBiases['adjust-regions'] = Number(
    ((adjustedBiases['adjust-regions'] ?? 0.5) + 0.05).toFixed(2),
  );
...
function optimizeLandingPage(
  ...
  if (negotiation && !negotiation.approved) {
    suggestions.push(
      'Multi-agent council vetoed the plan — consider simplifying landing page messaging and reducing budget risk.',
    );
  }
...
function detectCompetitorChange(prev: any, next: any) {
  ...
  if (next.backlinks !== prev.backlinks) {
    alerts.push(
      `Backlink profile delta: ${prev.backlinks} → ${next.backlinks} links`,
    );
  }
  ...
  return {
    ...
    alerts: alerts.length
      ? alerts
      : ['No posture changes detected; competitor posture stable.'],
  };
}
...
function allocateBudget(
  ...
  if (
    negotiation.votes &&
    negotiation.votes.filter((v: any) => v.approve).length === 2
  ) {
    recommendedShift = 0;
  }
...
function negotiatePlan(
  ...
  votes.push({
    ...
    confidence: 0.75,
    notes:
      simulatedReward - longReward > -0.25
...
async function saveSimulationState(env: Env, state: any) {
  ...
  await env.SIMULATION_STATE.put(
    `sim-${state.timestamp}`,
...
function computeLongHorizonReward(
  memory: any,
  growth: any,
  fusion: any,
): number {
  ...
  return Math.min(
    1,
    Math.max(0, Number((avgPerf + growthBias + fusionSignal).toFixed(2))),
  );
}
...
function strategicPlanner(
  ...
  let basePlan = [
    ...
    {
      type: 'adjust-budget',
      reason: 'Defensive cost protection: lowering daily spend',
    },
  ];

  if (longReward > 0.75) {
    basePlan = basePlan.filter(
      (p) => p.type === 'adjust-keywords' || p.type === 'adjust-regions',
    );
  } else if (longReward > 0.55) {
    basePlan = basePlan.filter(
      (p) => p.type === 'adjust-bidding' || p.type === 'adjust-keywords',
    );
  } else {
...
async function loadGrowthHistory(env: Env) {
        ...
        try {
          ...
          if (Array.isArray(regions) && regions.length) {
            const avg =
              regions.reduce(
                (s: number, r: any) =>
                  s + (r.forecast || r.interestScore || 70),
                0,
              ) / regions.length;
  ...
  return {
    backlinkCounts: backlinkCounts.length
      ? backlinkCounts
      : [12, 14, 18, 22, 28],
...
async function growthEngine(env: Env) {
  ...
  if (
    (backlinkTrend === 'strong-up' || backlinkTrend === 'up') &&
    (heatmapTrend === 'strong-up' || heatmapTrend === 'up')
  ) {
    bias = 'aggressive';
  } else if (
    backlinkTrend === 'down' ||
    heatmapTrend === 'down' ||
    backlinkTrend === 'strong-down'
  ) {
    bias = 'defensive';
  }
...
async function loadFusionContext(env: Env): Promise<FusionContext> {
  ...
  if (env.BACKLINKS) {
    ...
    for (const key of backlinkKeys.keys) {
      ...
      if (data) {
        try {
          backlinks.push(JSON.parse(data));
        } catch {
          backlinks.push(data);
        }
      }
  ...
  if (env.COMPETITORS) {
    ...
    for (const key of competitorKeys.keys) {
      ...
      if (data) {
        try {
          competitors.push(JSON.parse(data));
        } catch {
  ...
  const marlRaw = env.MARL_STATE
    ? await env.MARL_STATE.get('lastDecision')
    : null;
...
function computeFusionDecision(ctx: FusionContext): FusionDecision {
  ...
  const aggressiveCompetitors = ctx.competitors.filter(
    (c) => c.strategy && String(c.strategy).includes('Aggressive'),
  ).length;
  const hotCounties = Array.isArray(ctx.heatmap)
    ? ctx.heatmap.filter((r: any) => (r.forecast || r.interestScore || 0) > 75)
        .length
    : 0;
...
async function fusionEngine(
  env: Env,
): Promise<{ fusion: FusionDecision; ctx: FusionContext }> {
...
async function googleAdsRequest(
  ...
  if (!token || !env.GOOGLE_ADS_CUSTOMER_ID) {
    return {
      ...
      path,
      message:
        'Google Ads API credentials pending input - Actuator action simulated cleanly.',
    };
  }
  try {
...
async function applyKeywordAdjustment(env: Env, keywords: string[]) {
  if (!env.GOOGLE_ADS_CUSTOMER_ID)
    return { simulated: true, action: 'adjust-keywords', keywords };
  return googleAdsRequest(
    env,
    `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/googleAds:mutate`,
    'POST',
    {
...
async function applyRegionAdjustment(env: Env, regions: string[]) {
  if (!env.GOOGLE_ADS_CUSTOMER_ID)
    return { simulated: true, action: 'adjust-regions', regions };
  return googleAdsRequest(
    env,
    `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/googleAds:mutate`,
    'POST',
      ...
      operations: [
        {
          campaignCriterionOperation: {
            create: {
              location: { geoTargetConstant: regions },
              status: 'ENABLED',
            },
          },
        },
...
async function applyBudgetAdjustment(env: Env, amount: number) {
  if (!env.GOOGLE_ADS_CUSTOMER_ID)
    return { simulated: true, action: 'adjust-budget', amount };
  return googleAdsRequest(
    env,
    `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/googleAds:mutate`,
    'POST',
      ...
      operations: [
        {
          campaignBudgetOperation: {
            update: {
              resourceName: `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/campaignBudgets/1`,
...
async function applyBiddingStrategy(env: Env, strategy: string) {
  if (!env.GOOGLE_ADS_CUSTOMER_ID)
    return { simulated: true, action: 'adjust-bidding', strategy };
  return googleAdsRequest(
    env,
    `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/googleAds:mutate`,
    'POST',
      ...
      operations: [
        {
          campaignOperation: {
            update: {
              resourceName: `customers/${env.GOOGLE_ADS_CUSTOMER_ID}/campaigns/1`,
              biddingStrategyType: strategy,
            },
            updateMask: 'biddingStrategyType',
          },
        },
      ],
    },
...
function aiScoreBacklink(anchor: string, url: string): number {
  ...
  if (url.includes('gov') || url.includes('edu') || url.includes('.ie'))
    score += 0.2;
  if (
    anchor.toLowerCase().includes('retrofit') ||
    anchor.toLowerCase().includes('ecosmarthomes') ||
    anchor.toLowerCase().includes('seai')
  )
    score += 0.1;
...
async function marlCoordinator(
  env: Env,
  ctxInput: { lastAction: string; rewardScore: number },
) {
  const previous = env.MARL_STATE
    ? await env.MARL_STATE.get('lastDecision')
    : null;
  const historyReward = previous
    ? (JSON.parse(previous).modeledReward ?? 0.5)
    : 0.5;
  ...
  const rollbackScore =
    votes.filter((v) => v.rollback).reduce((sum, v) => sum + v.weight, 0) /
    (totalWeight || 1);

  const rollbackRequired = rollbackScore > 0.5;
  const modeledReward = Math.min(
    1,
    ctx.rewardScore * 0.7 + historyReward * 0.3,
  );
...
async function handleBacklinkDiscovery(
  env: Env,
  request: Request,
): Promise<Response> {
  try {
    const body = (await request
      .json()
      .catch(() => ({ targetUrl: 'https://ecosmarthomes.ie' }))) as any;
    ...
    let backlinks = html
      ? [...html.matchAll(/<a[^>]+href="([^"]+)"[^>]*>(.*?)<\/a>/gi)]
          .map((match) => ({
            url: match[1],
            anchor: match[2].replace(/<[^>]+>/g, '').trim(),
          }))
          .slice(0, 20)
      : [
          {
            url: 'https://seai.ie/contractors/ecosmarthomes',
            anchor: 'EcoSmartHomes SEAI Registered Retrofit Advisor',
          },
          {
            url: 'https://energy.ie/guide',
            anchor: 'Irish Home Energy Retrofit Guide 2026',
          },
        ];

    backlinks = backlinks.map((b) => ({
      ...b,
      aiScore: aiScoreBacklink(b.anchor, b.url),
    }));

    if (env.BACKLINKS) {
      await env.BACKLINKS.put(targetUrl, JSON.stringify(backlinks));
      await env.BACKLINKS.put(
        `crawl-${Date.now()}`,
        JSON.stringify({
          targetUrl,
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        endpoint: 'backlink-discovery',
        target: targetUrl,
        count: backlinks.length,
        backlinks,
        persisted: !!env.BACKLINKS,
      }),
    ...
    return new Response(
      JSON.stringify({
        ok: false,
        error: 'Failed to process backlink discovery',
        details: String(err),
      }),
...
async function handleCompetitorDiff(
  env: Env,
  request: Request,
): Promise<Response> {
  try {
    ...
    if (!Array.isArray(competitors) || competitors.length === 0) {
      return new Response(
        JSON.stringify({ ok: false, error: 'Missing competitors array' }),
    ...
    for (const domain of competitors) {
      try {
        ...
        const entry = {
          domain,
          contentSize: html.length || 18500,
          rankChange: html ? Math.round((html.length % 100) / 10) - 5 : +2,
          strategy,
        };
        if (env.COMPETITORS) {
          await env.COMPETITORS.put(
        ...
        results.push({
          domain,
          error: 'Failed to fetch competitor site',
          strategy: 'Unknown',
        });
      }
    }
    return new Response(
      JSON.stringify({
        ok: true,
        endpoint: 'competitor-diff',
        competitors: results,
        persisted: !!env.COMPETITORS,
      }),
      { headers: { 'Content-Type': 'application/json' } },
    );
  } catch (err) {
    return new Response(
      JSON.stringify({
        ok: false,
        error: 'Failed to process competitor diff',
        details: String(err),
      }),
      { status: 500, headers: { 'Content-Type': 'application/json' } },
    );
...
async function handleRegionalHeatmap(env: Env): Promise<Response> {
  ...
  let regions = counties.map((county) => ({
    county,
    interestScore: Math.floor(Math.random() * 40) + 60,
  }));
  regions = regions.map((r) => ({
    ...r,
    forecast: aiForecastInterest(r.interestScore),
  }));
  if (env.HEATMAP) {
  ...
  return new Response(
    JSON.stringify({
      ok: true,
      endpoint: 'regional-heatmap',
      generatedAt: new Date().toISOString(),
      regions,
      persisted: !!env.HEATMAP,
    }),
    { headers: { 'Content-Type': 'application/json' } },
  );
}

async function handleRollbackDecision(
  env: Env,
  request: Request,
): Promise<Response> {
  try {
    ...
    const rewardScore =
      typeof body.rewardScore === 'number' ? body.rewardScore : 0.85;
    const marlResult = await marlCoordinator(env, { lastAction, rewardScore });
    return new Response(
      JSON.stringify({
        ok: true,
        endpoint: 'marl-rollback-decision',
        marl: marlResult,
        persisted: !!env.MARL_STATE,
      }),
      { headers: { 'Content-Type': 'application/json' } },
    );
  } catch (err) {
    return new Response(
      JSON.stringify({
        ok: false,
        error: 'Failed to process MARL rollback decision',
        details: String(err),
      }),
      { status: 500, headers: { 'Content-Type': 'application/json' } },
    );
...
function mapEircodeToCounty(eircode: string): string {
  ...
  if (
    clean.startsWith('T12') ||
    clean.startsWith('T23') ||
    clean.startsWith('P51')
  )
    return 'Cork';
  if (
    clean.startsWith('D0') ||
...
function estimateBER(
  yearBuilt: number = 1998,
  _heating: string = 'Oil',
  insulation: string[] = [],
): { currentBER: string; projectedBER: string } {
  let currentBER = 'D2';
...
function computeSeaiEligibility(details: HomeownerDetails) {
  ...
  if (yearBuilt < 2021) {
    eligibleGrants.push({
      ...
      amount: 6500,
      description:
        'SEAI grant funding toward replacing fossil-fuel heating with an A-rated heat pump.',
  ...
  if (!insulation.includes('Attic')) {
    eligibleGrants.push({
      ...
      amount: 1700,
      description:
        'SEAI grant coverage for high-retention ceiling and joist insulation.',
  ...
  if (!insulation.includes('Walls')) {
    eligibleGrants.push({
      ...
      amount: 3200,
      description:
        'SEAI grant for continuous thermal envelope wall insulation.',
  ...
  eligibleGrants.push({
    ...
    amount: 2100,
    description:
      'SEAI zero-emissions rooftop solar electricity generation grant.',
  ...
  eligibleGrants.push({
    ...
    amount: 700,
    description:
      'SEAI grant for multi-zone smart thermostat and motorized valve upgrades.',
...
function computeGrantPlan(details: HomeownerDetails): GrantEligibilityRecord {
  ...
  const { currentBER, projectedBER } = estimateBER(
    yearBuilt,
    heating,
    insulation,
  );
...
async function saveGrantRecord(env: Env, record: GrantEligibilityRecord) {
  ...
  const history: string[] = existingHistoryRaw
...
async function getGrantRecord(
  env: Env,
  id: string,
): Promise<GrantEligibilityRecord | null> {
...
export default {
  async fetch(request: Request, env: Env, _ctx: any): Promise<Response> {
    ...
    if (
      ...
      url.pathname === '/seo/sitemap.xml' ||
      url.pathname === '/sitemaps/sitemap.xml'
    ) {
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>https://ecosmarthomes.ie/solar-pv-grants-ireland</loc><lastmod>2026-07-28</lastmod><changefreq>weekly</changefreq><priority>0.9</priority></url>
    ...
    if (url.pathname === '/api/status') {
      return new Response(
        JSON.stringify({
          ok: true,
          message:
            'EcoSmartHomes Autonomous Phase 22 Conflict Resolution Engine Active',
          phase: 22,
          squad: [
            'Risk Guard Agent',
            'Growth Opportunity Agent',
            'Efficiency Governor Agent',
          ],
          googleAdsStatus: env.GOOGLE_ADS_CUSTOMER_ID
            ? 'Configured & Live'
            : 'Pending Credentials (Simulated Mode Active)',
          features: [
            'Phase 23 Hybrid Grant Intelligence',
            'Autonomous Multi-Agent Conflict Resolution',
            'Ecosystem Intelligence Console',
            'Real-Time Competitor Watchdog',
          ],
        }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/grants/eligibility'
    ) {
      try {
        const body = (await request
          .json()
          .catch(() => ({}))) as HomeownerDetails;
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to compute grant eligibility',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      ...
      url.pathname.startsWith('/api/grants/plan/')
    ) {
      const planId = url.pathname
        .replace('/api/grants/plan/', '')
        .replace('/pdf', '');
      const plan =
        (await getGrantRecord(env, planId)) ||
        computeGrantPlan({ eircode: 'V94 X2C9' });
      const totalGrant = (plan.eligibleGrants || []).reduce(
        (sum, g) => sum + g.amount,
        0,
      );
...
<body>
  ...
  <script>window.onload = function() { window.print(); };</script>
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/grants/pdf-insights'
    ) {
      const downloadCountRaw = env.GRANTS_STATE
        ? await env.GRANTS_STATE.get('pdf_download_count')
        : null;
      const totalPdfDownloads = downloadCountRaw
    ...
    if (
      ...
      return new Response(
        JSON.stringify({ ok: false, error: 'Grant plan not found' }),
    ...
    if (request.method === 'GET' && url.pathname === '/api/grants/latest') {
      const raw = env.GRANTS_STATE
        ? await env.GRANTS_STATE.get('latest')
        : null;
      const plan = raw ? JSON.parse(raw) : computeGrantPlan({});
      return new Response(
        JSON.stringify({ ok: true, timestamp: Date.now(), plan }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }

    if (request.method === 'GET' && url.pathname === '/api/grants/history') {
      const historyRaw = env.GRANTS_STATE
        ? await env.GRANTS_STATE.get('history_list')
        : null;
      ...
      return new Response(
        JSON.stringify({ ok: true, count: records.length, history: records }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
      ...
      return new Response(
        JSON.stringify({
          ...
          paperworkBottlenecks: [
            ...
            {
              item: 'Pre-Upgrade BER Assessment Certificate',
              dropoffPercent: 24,
            },
          ...
          regionalDemand: [
            {
              county: 'Limerick',
              count: 48,
              topUpgrade: 'Heat Pump + Solar PV',
            },
            {
              county: 'Cork',
              count: 38,
              topUpgrade: 'Attic & Wall Insulation',
            },
            {
              county: 'Dublin',
              count: 32,
              topUpgrade: 'Air-to-Water Heat Pump',
            },
    ...
    if (request.method === 'GET' && url.pathname === '/api/grants/heatmap') {
      return new Response(
        JSON.stringify({
          ...
          heatmap: [
            {
              region: 'Munster (Limerick/Cork)',
              interestScore: 92,
              grantDemand: 'High',
            },
            {
              region: 'Leinster (Dublin/Kildare)',
              interestScore: 86,
              grantDemand: 'Very High',
            },
            {
              region: 'Connacht (Galway/Mayo)',
              interestScore: 78,
    ...
    if (request.method === 'POST' && url.pathname === '/api/advisor/book') {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        const booking_id = `book_${new Date()
          .toISOString()
          .replace(/[-:T.]/g, '')
          .slice(0, 12)}_${Math.floor(Math.random() * 9000 + 1000)}`;
        const record = {
        ...
        if (env.ADVISOR_BOOKINGS) {
          ...
          const listRaw = await env.ADVISOR_BOOKINGS.get(
          ...
          await env.ADVISOR_BOOKINGS.put(
            'booking_history_list',
        ...
        return new Response(
          JSON.stringify({
            ...
            booking_id,
            record,
            message: 'Advisor consultation booked successfully',
          }),
          { headers: { 'Content-Type': 'application/json' } },
        );
      } catch (err) {
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to create advisor booking',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/advisor/booking/')
    ) {
      const bookingId = url.pathname.replace('/api/advisor/booking/', '');
      const raw = env.ADVISOR_BOOKINGS
        ? await env.ADVISOR_BOOKINGS.get(bookingId)
        : null;
      if (raw) {
        return new Response(
          JSON.stringify({ ok: true, booking: JSON.parse(raw) }),
          { headers: { 'Content-Type': 'application/json' } },
        );
      }
      return new Response(
        JSON.stringify({ ok: false, error: 'Booking record not found' }),
        { status: 404, headers: { 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (request.method === 'GET' && url.pathname === '/api/advisor/bookings') {
      const listRaw = env.ADVISOR_BOOKINGS
        ? await env.ADVISOR_BOOKINGS.get('booking_history_list')
        : null;
      ...
      return new Response(
        JSON.stringify({ ok: true, count: bookings.length, bookings }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
          ...
          slots: [
            {
              time: '09:00 AM',
              status: 'booked',
              homeowner: 'Patrick Kelly (V94 X2C9)',
              date: '2026-08-06',
            },
            {
              time: '11:00 AM',
              status: 'booked',
              homeowner: "Sarah O'Connor (V94 H7T2)",
              date: '2026-08-06',
            },
            {
              time: '02:00 PM',
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/register') {
      try {
        ...
        if (env.HOMEOWNERS) {
          await env.HOMEOWNERS.put(
            `user_${userRecord.email}`,
        ...
        return new Response(
          JSON.stringify({ ok: true, user: userRecord, token }),
          {
            headers: {
              'Content-Type': 'application/json',
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/login') {
      try {
        ...
        const raw = env.HOMEOWNERS
          ? await env.HOMEOWNERS.get(`user_${email}`)
          : null;
        const user = raw
          ? JSON.parse(raw)
          : {
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/logout') {
      return new Response(JSON.stringify({ ok: true, message: 'Logged out' }), {
        headers: {
          'Content-Type': 'application/json',
          'Set-Cookie':
            'eso_session=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT',
        },
      });
    }
    ...
    if (request.method === 'GET' && url.pathname === '/api/auth/me') {
      const raw = env.HOMEOWNERS
        ? await env.HOMEOWNERS.get('latest_user')
        : null;
      const user = raw
        ? JSON.parse(raw)
        : {
    ...
    if (request.method === 'POST' && url.pathname === '/api/paperwork/upload') {
      try {
        ...
        if (env.HOMEOWNERS) {
          await env.HOMEOWNERS.put(
            `doc_${userId}_${docName}`,
            JSON.stringify(fileRecord),
          );
        }

        return new Response(JSON.stringify({ ok: true, fileRecord }), {
          headers: { 'Content-Type': 'application/json' },
        });
      } catch (err) {
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to upload paperwork',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/paperwork/')
    ) {
      return new Response(
        JSON.stringify({
          ok: true,
          documents: [
            {
              docName: 'Proof of Property Ownership (MPRN)',
              status: 'verified',
              uploadedAt: Date.now() - 7200000,
            },
            {
              docName: 'Recent Electricity Utility Bill',
              status: 'verified',
              uploadedAt: Date.now() - 3600000,
            },
            {
              docName: 'Pre-Upgrade BER Assessment Cert',
              status: 'pending',
              uploadedAt: null,
            },
            {
              docName: 'SEAI Contractor Sign-off Sheet',
              status: 'pending',
              uploadedAt: null,
            },
          ],
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/advisor/reschedule'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        return new Response(
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/homeowners/insights'
    ) {
      return new Response(
        JSON.stringify({
          ok: true,
          ...
          timelineMilestones: [
            ...
            {
              step: '2. Smart Heating Controls',
              completed: 62,
              inProgress: 34,
              pending: 28,
            },
            {
              step: '3. Heat Pump Upgrade',
              completed: 44,
              inProgress: 42,
              pending: 38,
            },
            {
              step: '4. Solar PV Panels',
              completed: 32,
              inProgress: 48,
              pending: 44,
            },
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/retrofit/generate'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        const grantRecord = env.GRANTS_STATE
          ? await env.GRANTS_STATE.get(grantId)
          : null;
        const userRecord = env.HOMEOWNERS
          ? await env.HOMEOWNERS.get(`uid_${userId}`)
          : null;

        const plan = aiPlanner(
          grantRecord ? JSON.parse(grantRecord) : null,
          userRecord ? JSON.parse(userRecord) : null,
        );

        if (env.RETROFIT_PLANS) {
          await env.RETROFIT_PLANS.put(plan.plan_id, JSON.stringify(plan));
          await env.RETROFIT_PLANS.put(
            `grant_${grantId}`,
            JSON.stringify(plan),
          );
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to generate AI retrofit plan',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/retrofit/plan/')
    ) {
      const parts = url.pathname.split('/');
      const grantId = parts[parts.length - 1];

      const raw = env.RETROFIT_PLANS
        ? await env.RETROFIT_PLANS.get(`grant_${grantId}`)
        : null;
      if (raw) {
    ...
    if (
      ...
      const rawPlan = env.RETROFIT_PLANS
        ? (await env.RETROFIT_PLANS.get(`grant_${planId}`)) ||
          (await env.RETROFIT_PLANS.get(planId))
        : null;
      const plan = rawPlan ? JSON.parse(rawPlan) : aiPlanner({ id: planId });
      const rawUser = env.HOMEOWNERS
        ? await env.HOMEOWNERS.get('latest_user')
        : null;
      const user = rawUser
        ? JSON.parse(rawUser)
        : { name: "Sarah O'Connor", eircode: 'V94 X2C9' };
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/retrofit/pdf-insights'
    ) {
      return new Response(
        JSON.stringify({
          ok: true,
    ...
    if (request.method === 'GET' && url.pathname === '/api/retrofit/plans') {
      const raw = env.RETROFIT_PLANS
        ? await env.RETROFIT_PLANS.get('latest_plan')
        : null;
      const plan = raw ? JSON.parse(raw) : aiPlanner();
      return new Response(JSON.stringify({ ok: true, plans: [plan] }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/contractors/match/')
    ) {
      const parts = url.pathname.split('/');
      const planId = parts[parts.length - 1];
      const matches = matchContractor('Heat Pump', 'Limerick');
      return new Response(
        JSON.stringify({ ok: true, plan_id: planId, matches }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/contractors/book') {
      try {
        ...
        return new Response(JSON.stringify({ ok: true, job: jobRecord }), {
          headers: { 'Content-Type': 'application/json' },
        });
      } catch (err) {
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to book contractor',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/api/jobs/')) {
      ...
      return new Response(JSON.stringify({ ok: true, jobs: [job] }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (request.method === 'GET' && url.pathname === '/api/contractors') {
      return new Response(
        JSON.stringify({ ok: true, contractors: SAMPLE_CONTRACTORS }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/grants/submit') {
      try {
        ...
        const payload = generateGrantSubmissionPayload(
          grantId,
          planId,
          userId,
          userRecord,
        ...
        if (env.GRANT_SUBMISSIONS) {
          await env.GRANT_SUBMISSIONS.put(
        ...
        return new Response(JSON.stringify({ ok: true, submission: payload }), {
          headers: { 'Content-Type': 'application/json' },
        });
      } catch (err) {
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to submit SEAI application',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/grants/submission/')
    ) {
      const parts = url.pathname.split('/');
      const subId = parts[parts.length - 1];
      const raw = env.GRANT_SUBMISSIONS
        ? await env.GRANT_SUBMISSIONS.get(subId)
        : null;
      const sub = raw ? JSON.parse(raw) : generateGrantSubmissionPayload();
      return new Response(JSON.stringify({ ok: true, submission: sub }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/grants/submissions/')
    ) {
      const parts = url.pathname.split('/');
      const userId = parts[parts.length - 1];
      const raw = env.GRANT_SUBMISSIONS
        ? await env.GRANT_SUBMISSIONS.get(`user_${userId}`)
        : null;
      const sub = raw ? JSON.parse(raw) : generateGrantSubmissionPayload();
      return new Response(JSON.stringify({ ok: true, submissions: [sub] }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/grants/submissions'
    ) {
      const raw = env.GRANT_SUBMISSIONS
        ? await env.GRANT_SUBMISSIONS.get('latest_submission')
        : null;
      ...
      const submissions = raw
        ? [JSON.parse(raw), defaultSubmissions[1]]
        : defaultSubmissions;
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/grants/submission/update'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        if (env.GRANT_SUBMISSIONS && subId) {
          ...
          updatedRecord = updateSubmissionLifecycleStatus(
            rec,
            newStatus,
            rejectionReason,
          );
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/grants/status/insights'
    ) {
      return new Response(
        JSON.stringify({
          ok: true,
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/postinstall/') &&
      url.pathname !== '/api/postinstall'
    ) {
      const parts = url.pathname.split('/');
      const userId = parts[parts.length - 1];
      const raw = env.POST_INSTALL
        ? await env.POST_INSTALL.get(`user_${userId}`)
        : null;
      const record = raw
        ? JSON.parse(raw)
        : generatePostInstallRecord(
            'sub_2026_08_03_1801',
            'grant_2026_08_03_1207',
            'plan_2026_08_03_1512',
            userId,
          );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/postinstall/ber/upload'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        const userId = body.user_id || 'user_2026_08_03_1412';
        ...
        record = updatePostInstallTimeline(
          record,
          'ber_uploaded',
          'Post-install BER cert uploaded (Rating A)',
        );

        if (env.POST_INSTALL) {
          await env.POST_INSTALL.put(
            record.postInstall_id,
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to upload BER cert',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/postinstall/ber/schedule'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        const userId = body.user_id || 'user_2026_08_03_1412';
        ...
        record = updatePostInstallTimeline(
          record,
          'ber_scheduled',
          `BER assessment booked for ${record.berAssessment.scheduled}`,
        );
    ...
    if (request.method === 'GET' && url.pathname === '/api/postinstall') {
      const raw = env.POST_INSTALL
        ? await env.POST_INSTALL.get('latest_record')
        : null;
      const defaultRecords = [
        generatePostInstallRecord(),
        generatePostInstallRecord(
          'sub_2026_08_03_1809',
          'grant_2026_08_03_1209',
          'plan_2026_08_03_1519',
          'user_2026_08_03_1419',
        ),
      ];
      const records = raw
        ? [JSON.parse(raw), defaultRecords[1]]
        : defaultRecords;
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/postinstall/update'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        if (env.POST_INSTALL && piId) {
          ...
          record = updatePostInstallTimeline(
            record,
            event,
            body.notes || `Event ${event} recorded`,
          );
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to update postinstall event',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/postinstall/approval'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        record = updatePostInstallTimeline(
          record,
          'seai_approved',
          body.notes || 'SEAI grant application approved (€22,100)',
          'advisor',
        );
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to approve SEAI grant',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/postinstall/payment'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        const piId = body.postInstall_id || 'pi_2026_08_03_1901';
        ...
        record = updatePostInstallTimeline(
          record,
          'seai_paid',
          body.notes || 'SEAI released €22,100 grant funds via EFT',
          'advisor',
        );
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to disburse SEAI grant payment',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/postinstall/notifications/')
    ) {
      const parts = url.pathname.split('/');
      const userId = parts[parts.length - 1];
      const raw = env.POST_INSTALL
      ...
      return new Response(
        JSON.stringify({ ok: true, notifications: record.notifications || [] }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/journey/') &&
      url.pathname !== '/api/journey' &&
      url.pathname !== '/api/journey/insights'
    ) {
      const parts = url.pathname.split('/');
      const userId = parts[parts.length - 1];
      const raw = env.JOURNEY_TIMELINE
        ? await env.JOURNEY_TIMELINE.get(`timeline_${userId}`)
        : null;
    ...
    if (request.method === 'POST' && url.pathname === '/api/journey/event') {
      try {
        ...
        const userId = body.user_id || 'user_2026_08_03_1412';
        const event =
          (body.event as JourneyEventType) || 'grant_eligibility_complete';
        ...
        return new Response(
          JSON.stringify({
            ok: false,
            error: 'Failed to append journey event',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (request.method === 'GET' && url.pathname === '/api/journey') {
      const raw = env.JOURNEY_TIMELINE
        ? await env.JOURNEY_TIMELINE.get('latest_timeline')
        : null;
      const record = raw ? JSON.parse(raw) : generateJourneyRecord();
      return new Response(
        JSON.stringify({ ok: true, record, records: [record] }),
        { headers: { 'Content-Type': 'application/json' } },
      );
    }
        ...
        JSON.stringify({
          ...
          metrics: {
            ...
            funnelStages: [
              ...
              {
                stage: 'PDF Blueprint Exported',
                count: 118,
                avgDuration: '0.2 days',
              },
              { stage: 'Advisor Booked', count: 116, avgDuration: '1.1 days' },
              {
                stage: 'Portal Onboarded',
                count: 114,
                avgDuration: '0.4 days',
              },
              {
                stage: 'AI Plan Synthesized',
                count: 112,
                avgDuration: '0.5 days',
              },
              {
                stage: 'Contractor Matched',
                count: 108,
                avgDuration: '2.1 days',
              },
              {
                stage: 'Retrofit Installed',
                count: 102,
                avgDuration: '4.5 days',
              },
              {
                stage: 'BER Cert Uploaded',
                count: 96,
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/contractors/score/update'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        if (!contractor_id || !metrics) {
          return new Response(
            JSON.stringify({ error: 'Missing contractor_id or metrics' }),
            { status: 400, headers: { 'Content-Type': 'application/json' } },
          );
        }
        ...
        return new Response(
          JSON.stringify({
            error: 'Failed to update contractor score',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname.startsWith('/api/contractors/score/')
    ) {
      const parts = url.pathname.split('/');
      ...
      if (!record) {
        return new Response(
          JSON.stringify({ error: 'Contractor score record not found' }),
          { status: 404, headers: { 'Content-Type': 'application/json' } },
        );
      }
      return new Response(JSON.stringify(record), {
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/contractors/scores'
    ) {
      const sampleIds = [
        'ctr_2026_08_03_1612',
        'ctr_2026_08_03_1619',
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/contractors/scores/insights'
    ) {
      const sampleIds = [
        'ctr_2026_08_03_1612',
        'ctr_2026_08_03_1619',
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/contractors/score/update-from-journey'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        const record = await updateContractorScoreFromJourney(
          env,
          contractor_id,
          user_id,
        );
        ...
        return new Response(
          JSON.stringify({
            error: 'Failed to update contractor score from journey',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/upgrades/recommendations/generate'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        const userId = body.user_id || 'user_2026_08_03_1412';
        ...
        return new Response(
          JSON.stringify({
            error: 'Failed to generate home upgrade recommendations',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/upgrades/recommendations'
    ) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/upgrades/all') {
      const sampleUserIds = [
        ...
        'user_2026_08_03_1415',
        'user_2026_08_03_1420',
      ];
      const bundles = [];
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/insights/national/generate'
    ) {
      try {
    ...
    if (
      request.method === 'POST' &&
      url.pathname === '/api/forecasting/generate'
    ) {
      try {
        const body = (await request.json().catch(() => ({}))) as any;
        ...
        return new Response(
          JSON.stringify({
            error: 'Failed to generate retrofit forecast',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (
      request.method === 'GET' &&
      (url.pathname === '/api/forecasting' ||
        url.pathname.startsWith('/api/forecasting/'))
    ) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/advisor/chat') {
      try {
        ...
        if (env.RETROFIT_ADVISOR_SESSIONS) {
          const raw = await env.RETROFIT_ADVISOR_SESSIONS.get(sessionKey, {
            type: 'json',
          });
          session = raw || {
            user_id: userId,
        ...
        if (env.RETROFIT_ADVISOR_SESSIONS) {
          await env.RETROFIT_ADVISOR_SESSIONS.put(
            sessionKey,
        ...
        return new Response(JSON.stringify({ reply, session }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      } catch (err) {
        return new Response(
          JSON.stringify({
            error: 'Failed to process advisor chat',
            details: String(err),
          }),
          { status: 500, headers: { 'Content-Type': 'application/json' } },
        );
    ...
    if (request.method === 'GET' && url.pathname === '/api/advisor/sessions') {
      ...
      if (env.RETROFIT_ADVISOR_SESSIONS) {
        for (const id of sampleUserIds) {
          const raw = await env.RETROFIT_ADVISOR_SESSIONS.get(`advisor_${id}`, {
            type: 'json',
          });
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/orchestrator/state'
    ) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/conflict/latest') {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('conflict-latest')
        : null;
      if (raw) {
        return new Response(raw, {
          headers: { 'Content-Type': 'application/json' },
    ...
    if (request.method === 'GET' && url.pathname === '/api/content/latest') {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('content-latest')
        : null;
      if (raw) {
        return new Response(raw, {
          headers: { 'Content-Type': 'application/json' },
    ...
    if (request.method === 'GET' && url.pathname === '/api/ecosystem/latest') {
      ...
      const heatmap = safeParse(
        env.HEATMAP ? await env.HEATMAP.get('latest') : null,
        { scrollDepth: 0.42, clickConcentration: 0.28 },
      );
      const fusion = safeParse(
        env.COMPETITORS ? await env.COMPETITORS.get('snapshot_latest') : null,
        {
          rank: 2,
          backlinks: 24,
          keywords: [
      ...
      const growth = safeParse(
        env.AUTONOMY_LOG ? await env.AUTONOMY_LOG.get('latest_growth') : null,
        { bias: 'balanced', backlinkTrend: 'up', heatmapTrend: 'strong-up' },
      );
      const strategy = safeParse(
        env.STRATEGY ? await env.STRATEGY.get('objectives') : null,
        DEFAULT_OBJECTIVES,
      );
      const simulation = safeParse(
        env.SIMULATION_STATE ? await env.SIMULATION_STATE.get('latest') : null,
        { simState: { cpcVolatility: 0.28, regionalDemandShock: 0.15 } },
      );
      const negotiation = safeParse(
        env.AUTONOMY_LOG
      ...
      const budget = safeParse(
        env.AUTONOMY_LOG ? await env.AUTONOMY_LOG.get('budget-latest') : null,
        { recommendedShift: 0.1, reason: 'Strong real + simulated reward.' },
      );
      const watchdog = safeParse(
        env.AUTONOMY_LOG ? await env.AUTONOMY_LOG.get('watchdog-latest') : null,
        {
          alerts: [
      ...
      const landing = safeParse(
        env.AUTONOMY_LOG ? await env.AUTONOMY_LOG.get('landing-latest') : null,
        {
          suggestions: [
    ...
    if (request.method === 'GET' && url.pathname === '/api/landing/latest') {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('landing-latest')
        : null;
      if (raw) {
        return new Response(raw, {
          headers: { 'Content-Type': 'application/json' },
    ...
    if (request.method === 'GET' && url.pathname === '/api/watchdog/latest') {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('watchdog-latest')
        : null;
      if (raw) {
        return new Response(raw, {
          headers: { 'Content-Type': 'application/json' },
    ...
    if (request.method === 'GET' && url.pathname === '/api/budget/latest') {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('budget-latest')
        : null;
      if (raw) {
        return new Response(raw, {
          headers: { 'Content-Type': 'application/json' },
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/api/negotiation/latest'
    ) {
      const raw = env.AUTONOMY_LOG
        ? await env.AUTONOMY_LOG.get('negotiation-latest')
        : env.MARL_STATE
    ...
    if (url.pathname === '/api/seo/backlink-discovery')
      return handleBacklinkDiscovery(env, request);
    if (url.pathname === '/api/seo/competitor-diff')
      return handleCompetitorDiff(env, request);
    if (url.pathname === '/api/analytics/regional-heatmap')
      return handleRegionalHeatmap(env);
    if (url.pathname === '/api/marl/rollback-decision')
      return handleRollbackDecision(env, request);
    ...
    if (url.pathname === '/api/strategy/history') {
      const memory = await loadStrategicMemory(env);
      const objectivesRaw = env.STRATEGY
        ? await env.STRATEGY.get('objectives')
        : null;
    ...
    if (url.pathname === '/api/growth/history') {
      ...
      for (const key of keys.keys) {
        if (key.name.startsWith('growth-') || key.name === 'latest_growth') {
          ...
          if (data) {
            try {
              results[key.name] = JSON.parse(data);
            } catch {
    ...
    if (url.pathname === '/api/analytics/heatmap-history') {
      ...
      return new Response(data || JSON.stringify({ ok: true, regions: [] }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (url.pathname.startsWith('/api/')) {
      return new Response(
        JSON.stringify({
          ok: true,
          message: 'EcoSmartHomes API Edge Worker Active',
        }),
        { status: 200, headers: { 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (
      url.pathname === '/heat-pump-costs-ireland' ||
      url.pathname === '/solar-pv-grants-ireland' ||
      url.pathname === '/estimator' ||
      ...
      const pageDesc =
        'Calculate domestic heating and hot water costs, evaluate SEAI heat pump & solar PV grant savings, and find certified installers in Ireland.';
...
<body>
  ...
  async scheduled(_event: any, env: Env, _ctx: any) {
    ...
    await marlCoordinator(env, {
      lastAction: 'scheduled-cron-loop',
      rewardScore: 0.82,
    });
      ...
      for (const step of plan) {
        ...
        if (step.type === 'adjust-keywords') {
          action = actionAdjustKeywords(
            [
              'retrofit ireland 2026',
              'heat pump cost limerick V94',
              'SEAI solar PV grants dublin',
            ],
            step.reason,
          );
          await applyKeywordAdjustment(env, action.payload.newKeywords);
        } else if (step.type === 'adjust-regions') {
          action = actionAdjustRegions(
    ...
    if (env.BACKLINKS)
      await env.BACKLINKS.put(
        ...
        JSON.stringify({
          timestamp,
          note: 'Scheduled backlink crawl executed',
        }),
      );
    if (env.COMPETITORS)
      await env.COMPETITORS.put(
        ...
        JSON.stringify({
          timestamp,
          note: 'Scheduled competitor diff executed',
        }),
      );
    if (env.HEATMAP) {
      ...
      const regions = counties.map((c) => {
        ...
        return {
          county: c,
          interestScore: score,
          forecast: aiForecastInterest(score),
        };
      });
