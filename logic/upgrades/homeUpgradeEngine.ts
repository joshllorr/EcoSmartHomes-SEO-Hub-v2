...
export function deriveUpgradesFromJourney(
  timeline: JourneyTimeline,
): HomeUpgradeRecommendation[] {
  const events = timeline.events || [];
  const recs: HomeUpgradeRecommendation[] = [];
  const now = Date.now();
  const userId = timeline.user_id;

  const hasSolar = events.some(
    (e) =>
      e.event === 'seai_paid' ||
      e.event === 'installation_complete' ||
      (e.notes && e.notes.toLowerCase().includes('solar')),
  );
  const hasHeatPump = events.some(
    (e) =>
      e.event === 'seai_paid' ||
      (e.notes && e.notes.toLowerCase().includes('heat pump')),
  );
  const hasInsulation = events.some(
    (e) =>
      e.event === 'seai_paid' ||
  ...
  if (
    hasSolar &&
    !events.some((e) => e.notes && e.notes.toLowerCase().includes('battery'))
  ) {
    recs.push({
  ...
  if (!hasSolar) {
    recs.push({
      ...
      description:
        'Complement your heating system with clean rooftop solar electricity, supported by the SEAI €3,000 grant offset and hot water diverter.',
      estimatedCost: 6500,
  ...
  if (
    !events.some((e) => e.notes && e.notes.toLowerCase().includes('controls'))
  ) {
    recs.push({
...
export async function generateHomeUpgradeBundle(
  env: any,
  user_id: string,
): Promise<HomeUpgradeBundle> {
  const now = Date.now();
  let timeline: JourneyTimeline | null = null;

  if (env.JOURNEY_TIMELINE) {
    const raw = await env.JOURNEY_TIMELINE.get(`timeline_${user_id}`, {
      type: 'json',
    });
    ...
    timeline = {
      ...
      events: [
        ...
        { event: 'contractor_assigned', at: now - 500000000 },
        {
          ...
          at: now - 300000000,
          notes: 'Heat Pump & Solar PV installed',
        },
  ...
  if (env.HOME_UPGRADE_RECOMMENDATIONS) {
    ...
    await env.HOME_UPGRADE_RECOMMENDATIONS.put(
      'latest_bundle',
...
export async function getHomeUpgradeBundle(
  env: any,
  user_id: string,
): Promise<HomeUpgradeBundle> {
  if (env.HOME_UPGRADE_RECOMMENDATIONS) {
    const raw = await env.HOME_UPGRADE_RECOMMENDATIONS.get(user_id, {
      type: 'json',
    });
