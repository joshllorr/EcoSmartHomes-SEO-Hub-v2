...
export function deriveContractorMetricsFromJourney(events: JourneyEvent[]): Partial<ContractorMetrics> {
  const installationDuration = getDuration(events, "contractor_assigned", "installation_complete");
  const berUploadDuration = getDuration(events, "installation_complete", "ber_uploaded");
  const grantApprovalDuration = getDuration(events, "grant_submitted", "seai_approved");
  ...
  if (berUploadDuration !== null) {
    ...
    metrics.paperworkAccuracy = Math.max(0, Math.min(100, Math.round(100 - days * 4))); // lose 4 points per day
  }
  ...
  if (grantApprovalDuration !== null) {
    ...
    metrics.grantApprovalRate = Math.max(0, Math.min(100, Math.round(100 - days * 3))); // lose 3 points per day
  }
  ...
  if (installationDuration !== null) {
    ...
    if (days > expectedTimeline.installation) adherenceScore -= (days - expectedTimeline.installation) * 2;
  }

  if (berUploadDuration !== null) {
    const days = berUploadDuration / (1000 * 60 * 60 * 24);
    if (days > expectedTimeline.berUpload) adherenceScore -= (days - expectedTimeline.berUpload) * 2;
  }

  if (grantApprovalDuration !== null) {
    const days = grantApprovalDuration / (1000 * 60 * 60 * 24);
    if (days > expectedTimeline.approval) adherenceScore -= (days - expectedTimeline.approval) * 1.5;
  }

  metrics.timelineAdherence = Math.max(0, Math.min(100, Math.round(adherenceScore)));
