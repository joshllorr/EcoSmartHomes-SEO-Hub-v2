...
export async function generateCoachMessages(env: any, user_id: string): Promise<CoachMessageBundle> {
...
export async function getCoachMessages(env: any, user_id: string): Promise<CoachMessageBundle> {
  if (env.RETROFIT_COACH_MESSAGES) {
    const raw = await env.RETROFIT_COACH_MESSAGES.get(user_id, { type: "json" });
