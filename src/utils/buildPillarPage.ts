// src/utils/buildPillarPage.ts
export async function buildPillarPage(
  topic: any,
  tone: string = 'professional',
  audience: string = 'Irish homeowners',
  apiKey: string = '',
): Promise<string> {
  const topicTitle =
    typeof topic === 'string'
      ? topic
      : topic?.title || 'BER Rating Ireland & Retrofit Master Guide';
    ...
    JSON.stringify(
      {
        ...
        support_pages: [
          ...
          'SEAI Grant & Investment Calculator',
        ],
        word_count: 2200,
      },
      null,
