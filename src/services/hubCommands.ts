...
export async function sendHubCommand(action: CommandAction, payload: Record<string, unknown> = {}): Promise<void> {
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[Hub Command] ${action} failed with status ${res.status}:`, errorText);
    }
  } catch (err: unknown) {
...
export const hubCommands = {
  ...
  setDomainAutonomyMode: async (siteId: string, mode: "passive" | "assisted" | "full_autonomous") => {
    try {
      const res = await fetch("/api/autonomous-decisions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("[Autonomous Engine] Failed to set domain autonomy mode:", err);
    }
  }
};
