...
export const harborSync = {
  articleDraft: (slug: string, extra?: Partial<HarborPayload>) =>
    syncToHarbor({
      type: 'article_draft',
      slug,
      message: `New article draft created: ${slug}`,
      ...extra,
    }),
  ...
  rewriteSuccess: (
    slug: string,
    newGrade: string,
    extra?: Partial<HarborPayload>,
  ) =>
    syncToHarbor({
      type: 'rewrite_success',
      slug,
      newGrade,
      message: `Rewrite success: ${slug} upgraded to Grade ${newGrade}`,
      ...extra,
    }),
  ...
  serpDiffPatch: (
    ...
    syncToHarbor({
      type: 'serp_diff_patch',
      slug,
      topicsAdded,
      message: `SERP diff: Added ${topicsAdded} missing competitor topics to ${slug}`,
      ...extra,
    }),

  linkBaitGenerated: (slug: string, extra?: Partial<HarborPayload>) =>
    syncToHarbor({
      type: 'link_bait_generated',
      slug,
      message: `Link-bait hook generated for: ${slug}`,
      ...extra,
    }),
  ...
  autonomousExpansion: (gaps: string[], extra?: Partial<HarborPayload>) =>
    syncToHarbor({
      ...
      gaps,
      message: `Autonomous expansion: queued ${gaps.length} new cluster page(s)`,
      ...extra,
    }),
  ...
  visibilitySpike: (
    ...
    syncToHarbor({
      ...
      increment,
      source,
      message: `Visibility spike: +${increment} visits — ${source}`,
      ...extra,
    }),
  ...
  semanticEnrichment: (
    ...
    syncToHarbor({
      ...
      slug,
      entities,
      message: `Semantic enrichment: Added ${entities.join(', ')} to ${slug}`,
      ...extra,
    }),
  ...
  multiSiteExpansion: (
    ...
    syncToHarbor({
      ...
      gaps,
      domains,
      message: `Multi-site expansion: Created ${gaps.length} page(s) across ${domains.length} fleet domains`,
      ...extra,
    }),
