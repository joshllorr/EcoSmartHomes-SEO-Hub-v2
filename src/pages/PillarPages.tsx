...
export default function PillarPages() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      let meta: any = {
        ...
        support_pages: ["Heat Pump Readiness Assessment", "SEAI Grant Calculator"]
      };
      let markdown = output;
  ...
  return (
    <div className="p-8 space-y-6 text-left" id="pillar-pages-view">
      <h1 className="text-3xl font-bold text-white tracking-tight">Pillar Page Builder</h1>

      <div className="bg-white p-6 rounded-xl shadow-lg space-y-4 text-slate-900 border border-slate-200">
        <div>
          <label className="block text-sm font-medium mb-1 text-slate-800">Pillar topic</label>
          <input
            value={topic}
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium mb-1 text-slate-800">Tone</label>
            <select
              value={tone}
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium mb-1 text-slate-800">Audience</label>
            <input
              value={audience}
      ...
      <div className="space-y-4">
        {pillarPages.map((p: any, i: number) => (
          <div key={i} className="bg-white p-6 rounded-xl shadow-lg space-y-2 text-slate-900 border border-slate-200">
            <h2 className="text-xl font-semibold text-slate-900">{p.pillar_topic}</h2>
