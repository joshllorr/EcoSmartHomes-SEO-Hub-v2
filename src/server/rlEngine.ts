...
let agentPolicies: Record<AgentId, PolicyState> = JSON.parse(
  JSON.stringify(DEFAULT_AGENT_POLICIES),
);
let memoryExperiences: ExperienceRecord[] = [];

const MARL_MEMORY_FILE_PATH = path.join(
  process.cwd(),
  'data',
  'marl_memory.json',
);
...
export function detectAgentId(slug: string, siteId = ''): AgentId {
  ...
  if (target.includes('heat-pump') || target.includes('heatpump'))
    return 'heat-pumps';
  if (
    target.includes('solar') ||
    target.includes('pv') ||
    target.includes('battery')
  )
    return 'solar';
  if (
    target.includes('insulation') ||
    target.includes('attic') ||
    target.includes('wall')
  )
    return 'insulation';
  if (
    target.includes('grant') ||
    target.includes('seai') ||
    target.includes('subsidy')
  )
    return 'grants';
  return 'default';
}
  ...
  try {
    ...
    if (fs.existsSync(MARL_MEMORY_FILE_PATH)) {
      ...
      if (Array.isArray(parsed.experiences))
        memoryExperiences = parsed.experiences;
      console.log(
        `[MARL Engine] Loaded ${memoryExperiences.length} experiences & per-agent policy weights.`,
      );
    } else {
      saveMARLMemory();
    }
    ...
    console.error(
      '[MARL Engine] Failed to initialize persistent memory store:',
      err,
    );
  }
}
...
export function calculateAgentReward(
  ...
  const impressionsDelta =
    (after.impressions - before.impressions) / Math.max(before.impressions, 1);
  ...
  switch (agentId) {
    case 'heat-pumps':
      ...
      rawReward = ctrDelta * 10.0 + serpDelta * 0.2 + impressionsDelta * 0.1;
      break;

    case 'solar':
      // Agent B: High Reward for Backlink growth & Impressions
      rawReward =
        backlinksDelta * 0.6 + impressionsDelta * 1.5 + ctrDelta * 2.0;
      break;
    ...
    default:
      rawReward =
        ...
        serpDelta * 0.1 +
        backlinksDelta * 0.3 +
        impressionsDelta * 0.2;
  }

  // Action-specific modifier per agent
  if (action === 'link_bait' && agentId === 'solar') rawReward += 0.25;
  if (action === 'rewrite_article' && agentId === 'heat-pumps')
    rawReward += 0.25;
  if (action === 'queue_expansion' && agentId === 'insulation')
    rawReward += 0.25;
...
export function updateAgentPolicy(
  ...
  const policy = agentPolicies[agentId] || {
    ...DEFAULT_AGENT_POLICIES['default'],
    agentId,
  };
  ...
  policy.ctrTrendWeight = Math.max(
    0.05,
    Math.min(0.95, parseFloat((policy.ctrTrendWeight * decay).toFixed(4))),
  );
  policy.serpVolatilityWeight = Math.max(
    0.05,
    Math.min(
      0.95,
      parseFloat((policy.serpVolatilityWeight * decay).toFixed(4)),
    ),
  );
  policy.backlinkGapWeight = Math.max(
    0.05,
    Math.min(0.95, parseFloat((policy.backlinkGapWeight * decay).toFixed(4))),
  );
  policy.pillarWeaknessWeight = Math.max(
    0.05,
    Math.min(
      0.95,
      parseFloat((policy.pillarWeaknessWeight * decay).toFixed(4)),
    ),
  );
  policy.contentVelocityWeight = Math.max(
    0.05,
    Math.min(
      0.95,
      parseFloat((policy.contentVelocityWeight * decay).toFixed(4)),
    ),
  );
 ...
 */
export function runMultiAgentExperienceReplay(
  targetAgentId?: AgentId,
  sampleSize = 25,
): { replayCount: number; policies: Record<AgentId, PolicyState> } {
  ...
  const selected = samples
    ...
    .slice(0, Math.min(sampleSize, samples.length));
  ...
  console.log(
    `[MARL Engine] Completed Experience Replay on ${selected.length} memories for agent: ${targetAgentId || 'all'}.`,
  );
...
export function getAgentPerformance(agentId: AgentId): AgentPerformance {
  ...
  const efficiencyScore = Math.min(
    100,
    Math.max(0, Math.round((avgReward + 1) * 50)),
  );

  let smartnessLevel = 'Novice Strategy';
  if (experiences.length >= 3 && avgReward > 0.1)
    smartnessLevel = 'Proficient Strategy';
  if (experiences.length >= 6 && avgReward > 0.3)
    smartnessLevel = 'Master SEO Autopilot';
...
export function getMARLMemoryExperiences(
  agentId?: AgentId,
  limit = 50,
): ExperienceRecord[] {
