...
export type AutonomyMode = "passive" | "assisted" | "full_autonomous" | "paused";
 ...
 */
export function setDomainAutonomyMode(siteId: string, mode: AutonomyMode): Record<string, AutonomyMode> {
...
export function runPreFlightChecklist(
  ...
  const noManualOverride = !engineState.lastManualOverrideAt || (now - engineState.lastManualOverrideAt > 2 * 60 * 1000);
  ...
  if (!queueLengthReasonable) failedChecks.push(`Queue length exceeded (${currentQueueLength}/${maxQueueLength})`);
  if (!noConflictingTasks) failedChecks.push("Task conflict currently executing");
  ...
  if (!noCooldownViolation) {
    ...
    failedChecks.push(`Action Cooldown Active (${d.action} limited to 1x per ${cooldownMinutes}m, last ran ${elapsedMins}m ago)`);
  }
...
export function calculateTrendVectors(): TrendVectors {
  engineState.trendVectors = {
    ...
    serpVolatilityTrend: parseFloat((0.10 + Math.random() * 0.06).toFixed(3)),
...
export function evalPredictiveRules(trends: TrendVectors): Decision[] {
  ...
  if (trends.ctrTrend < -0.05 && trends.serpVolatilityTrend > 0.1) {
    ...
    const confidence = Math.min(0.99, parseFloat((0.88 * (policy.ctrTrendWeight / 0.30)).toFixed(2)));
    decisions.push({
      id: `pred-${now}-1`,
  ...
  if (trends.backlinkGrowthTrend < 0.1) {
    const agentId = detectAgentId("solar-pv-grants-ireland", "future-site-1.ie");
    const policy = getAgentRLPolicy(agentId);
    const confidence = Math.min(0.99, parseFloat((0.82 * (policy.backlinkGapWeight / 0.20)).toFixed(2)));
    decisions.push({
      id: `pred-${now}-2`,
      action: "link_bait",
  ...
  if (trends.pillarStrengthTrend < 0.02 && trends.contentVelocityTrend < 0) {
    ...
    const policy = getAgentRLPolicy(agentId);
    const confidence = Math.min(0.99, parseFloat((0.76 * (policy.contentVelocityWeight / 0.10)).toFixed(2)));
    decisions.push({
      id: `pred-${now}-3`,
...
export async function loadPageSignals(): Promise<PageSignal[]> {
  ...
  for (const siteId of domains) {
    for (const slug of sampleSlugs) {
      const hash = (siteId + slug).split("").reduce((acc, char) => acc + char.charCodeAt(0), 0);
...
export async function executeDecisions(
  ...
  for (const d of decisions) {
    ...
    if (mode === "passive") {
      d.executed = false;
      d.executionResult = "PASSIVE MODE: Suggestion logged (Auto-execution disabled)";
    } else if (mode === "assisted") {
      d.executed = false;
      d.executionResult = "ASSISTED MODE: Queued for Human Approval";
    } else if (mode === "full_autonomous") {
      // 1. Confidence & Priority Gate
      if (d.priority >= minPriority && d.confidence >= minConfidence && !d.executed) {
        ...
        if (!preFlight.passed) {
          console.warn(`[Pre-Flight & Cooldown] Action DEFERRED for ${d.siteId}/${d.slug} (${d.action}): ${preFlight.failureReason}`);
          d.executed = false;
          ...
          (async () => {
            try {
              console.log(`[Predictive Engine] Full Autonomous Executing (${d.action}): ${d.siteId}/${d.slug}`);
              let res: unknown;
              switch (d.action) {
                case "generate_draft":
                  res = await runDraftGenerator({ slug: d.slug, siteId: d.siteId } as DraftPayload);
                  break;
                case "rewrite_article":
                  res = await runRewriteEngine({ slug: d.slug, siteId: d.siteId } as RewritePayload);
                  break;
                case "queue_expansion":
                  res = await queueExpansion({ slug: d.slug, siteId: d.siteId } as ExpansionPayload);
                  break;
                case "link_bait":
                  res = await runLinkBaitGenerator({ slug: d.slug, siteId: d.siteId } as LinkBaitPayload);
                  break;
                case "publish":
                  res = await publishToGitHub({ slug: d.slug, siteId: d.siteId } as PublishPayload);
                  break;
              }
              d.executionResult = JSON.stringify(res || { ok: true });
            } catch (err) {
              const errorMsg = err instanceof Error ? err.message : String(err);
              console.error(`[Predictive Engine] Failed to execute ${d.action}:`, errorMsg);
    ...
    if (!engineState.decisionsHistory.some(existing => existing.id === d.id)) {
 ...
 */
export async function runDecisionCycle(): Promise<{ trends: TrendVectors; signals: PageSignal[]; decisions: Decision[]; executed: Decision[] }> {
  ...
  const coordinated = evaluateProposals(rawDecisions, engineState.executingTaskKeys.size);

  let executed: Decision[] = [];
  if (engineState.autoPilotEnabled && coordinated.approvedForExecution.length > 0) {
    executed = await executeDecisions(coordinated.approvedForExecution, 0.7, engineState.minConfidenceThreshold);
  } else {
