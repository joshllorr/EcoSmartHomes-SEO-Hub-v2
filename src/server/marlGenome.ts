...
let agentGenomes: Record<AgentId, PersonalityGenome> = JSON.parse(
  JSON.stringify(DEFAULT_AGENT_GENOMES),
);
...
export function initMARLGenomes(): void {
  try {
    ...
    if (fs.existsSync(GENOMES_FILE_PATH)) {
      ...
      console.log(
        `[MARL Genome] Loaded persistent Agent Personality DNA Genomes from disk.`,
      );
    } else {
      saveMARLGenomes();
    }
...
export function saveMARLGenomes(): void {
  try {
    ...
    fs.writeFileSync(
      GENOMES_FILE_PATH,
      JSON.stringify(agentGenomes, null, 2),
      'utf-8',
    );
  } catch (err) {
...
export function getAgentGenome(
...
export function getEmergentStrategicIdentity(
  ...
  if (aggression > 0.65 && rewriteBias > 0.6) {
    return {
  ...
  if (collaboration > 0.65 && linkbaitBias > 0.6) {
    return {
  ...
  if (caution > 0.65 || riskTolerance < 0.3) {
    return {
  ...
  if (curiosity > 0.5 || publishBias > 0.6) {
    return {
 ...
 */
export function checkAndTriggerStabilization(
  agentId: AgentId,
  genome: PersonalityGenome,
): void {
  const now = Date.now();
  ...
  if (genome.aggression > 0.9 || genome.rewriteBias > 0.9) {
    genome.aggression = 0.7;
    genome.rewriteBias = 0.7;
  ...
  if (genome.caution > 0.9 || genome.riskTolerance < 0.08) {
    genome.caution = 0.65;
 ...
 */
export function evolveAgentGenome(
  agentId: AgentId,
  reward: number,
  action: string,
): PersonalityGenome {
  const g = agentGenomes[agentId] || {
    ...DEFAULT_AGENT_GENOMES['default'],
    agentId,
  };
  const learningRate = 0.05;
  const influence =
    TRAIT_INFLUENCE_MAP[action] || TRAIT_INFLUENCE_MAP['rewrite_article'];
  ...
  for (const trait of traitNames) {
    ...
    const updatedTrait = (g[trait] + delta) * 0.995;

    // Apply Trait Caps: clamp(trait, 0.05, 0.95)
    g[trait] = Math.max(
      0.05,
      Math.min(0.95, parseFloat(updatedTrait.toFixed(3))),
    );
  }
...
export function runPersonalityShapingCycle(sampleSize = 30): {
  ...
  console.log(
    `[Personality Shaping Cycle] Replayed ${experiences.length} long-term memories across all Pillar Agent Genomes.`,
  );
