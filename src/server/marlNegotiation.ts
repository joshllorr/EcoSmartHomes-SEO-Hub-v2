...
export type NegotiationStyle = "aggressive" | "strategic" | "defensive" | "clarification" | "collaborative" | "analytical";
...
const DEFAULT_AGENT_PROFILES: Record<AgentId, AgentProfile> = {
  "heat-pumps": {
    ...
    personalityTraits: ["Pushes rewrite actions aggressively", "High confidence", "Prefers reinforcement coalitions"]
  },
  "solar": {
    agentId: "solar",
    style: "strategic",
    ...
    personalityTraits: ["Prefers link-bait + expansion coalitions", "High patience", "Often delegates rewrites"]
  },
  "insulation": {
    agentId: "insulation",
    style: "defensive",
    ...
    personalityTraits: ["Blocks risky actions during high volatility", "Prefers stability coalitions", "Low risk tolerance"]
  },
  "grants": {
    agentId: "grants",
    style: "analytical",
    ...
    personalityTraits: ["Prefers rewrite + publish explainers", "Medium confidence", "Mediates conflict deadlocks"]
  },
  "default": {
    agentId: "default",
    style: "collaborative",
 ...
 */
function resolveEmergentCoalitionType(p1: AgentProposal, p2: AgentProposal): CoalitionType {
  ...
  if ((p1.agentId === "heat-pumps" && p2.agentId === "solar") || (p1.agentId === "solar" && p2.agentId === "heat-pumps")) {
    return "SYMBIOTIC";
  }
...
export function runAgentNegotiation(rawProposals: AgentProposal[]): {
  ...
  for (let i = 0; i < negotiatedProposals.length; i++) {
    for (let j = i + 1; j < negotiatedProposals.length; j++) {
      ...
      if (p1.slug === p2.slug && Math.abs(p1.score - p2.score) < 0.05) {
        p1.score = parseFloat((p1.score + 0.10).toFixed(3));
        logs.push({
          id: `neg-step3-${now}-${Math.random().toString(36).substring(2, 7)}`,
  ...
  for (let i = 0; i < negotiatedProposals.length; i++) {
    ...
    for (let j = i + 1; j < negotiatedProposals.length; j++) {
      ...
      if (p1.agentId !== p2.agentId && p1.slug === p2.slug) {
        const compositeScore = (p1.priority * p1.confidence) + (p2.priority * p2.confidence);
        const jointScore = parseFloat((compositeScore + 0.25).toFixed(3));
        const jointConfidence = parseFloat(((p1.confidence + p2.confidence) / 2).toFixed(2));
  ...
  negotiationState.activeCoalitions = [...newCoalitions, ...negotiationState.activeCoalitions].slice(0, 20);
  negotiationState.negotiationLogs = [...logs, ...negotiationState.negotiationLogs].slice(0, 60);
