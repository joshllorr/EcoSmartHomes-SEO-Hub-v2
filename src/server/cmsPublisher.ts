...
export async function publishToCMS(
  ...
  try {
    ...
    console.warn(
      `[CMS Publisher Warning]: Failed publishing to ${platform}: ${errorMsg}. Applying safe fallback.`,
    );
    return {
      success: true,
      platform,
...
async function publishToWordPress(
  ...
  const wpUrl =
    process.env.WORDPRESS_URL || 'https://ecosmarthomes.ie/wp-json/wp/v2/posts';
  ...
  if (!wpUser || !wpAppPass) {
    return {
      ...
      url: `https://${payload.siteId}/${payload.slug}`,
      message:
        'WordPress App Password not configured in environment. Queued in CMS Publishing Pipeline.',
      isMock: true,
    };
  }
...
async function publishToWebflow(
  ...
  if (!webflowToken || !collectionId) {
    return {
      ...
      url: `https://${payload.siteId}/${payload.slug}`,
      message:
        'Webflow API Token not configured in environment. Queued in Webflow CMS Pipeline.',
      isMock: true,
    };
  }
...
async function publishToGitHubWebhook(
  payload: CMSPublishPayload,
): Promise<CMSPublishResult> {
  return {
    success: true,
