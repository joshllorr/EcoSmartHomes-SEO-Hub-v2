...
export function generatePostInstallRecord(
  ...
  const postInstallId = `pi_${new Date()
    .toISOString()
    .replace(/[-:T.]/g, '')
  ...
  const timeline: PostInstallTimelineEvent[] = [
    {
      ...
      at: now - 172800000,
      notes: 'Heat Pump & Solar PV installation verified',
      triggeredBy: 'system',
    },
    {
      event: 'ber_scheduled',
      at: now - 86400000,
      notes: "BER Assessor John O'Donnell booked for 2026-08-12",
      triggeredBy: 'advisor',
    },
    {
      event: 'ber_uploaded',
      at: now - 43200000,
      notes: 'Post-install BER cert uploaded (Achieved Rating: A)',
      triggeredBy: 'homeowner',
    },
    {
      event: 'seai_review',
      at: now - 21600000,
      notes: 'SEAI auditor acknowledged receipt under SEAI-2026-89412',
      triggeredBy: 'seai_cron',
    },
    {
      event: 'seai_approved',
      at: now - 7200000,
      notes: 'SEAI grant funding approved (€22,100)',
      triggeredBy: 'seai_cron',
    },
    {
      event: 'seai_paid',
      at: now,
      notes: 'EFT Grant payment released to homeowner bank account',
      triggeredBy: 'seai_cron',
    },
  ];
  ...
  const notifications: PostInstallNotification[] = [
    {
      ...
      message:
        '€22,100 grant funding has been transferred directly to your bank account via SEAI EFT.',
      timestamp: now,
      channels: ['portal', 'email', 'sms'],
      read: false,
    },
...
export function updatePostInstallTimeline(
  ...
  record.notifications.push({
    ...
    message:
      notes ||
      `Milestone ${event} reached for application ${record.submission_id}`,
    timestamp: now,
    channels: ['portal', 'email', 'sms'],
    read: false,
  });
...
export function cronSyncPostInstallRecord(
  record: PostInstallRecord,
): PostInstallRecord {
  const now = Date.now();
  ...
  if (record.seaiReview.status === 'pending') {
    record = updatePostInstallTimeline(
      record,
      'seai_review',
      '6-hour cron sync: SEAI acknowledged receipt',
      'seai_cron',
    );
  } else if (
    record.seaiReview.status === 'under_review' &&
    record.seaiApproval.status === 'pending'
  ) {
    record = updatePostInstallTimeline(
      record,
      'seai_approved',
      '6-hour cron sync: SEAI approved €22,100 grant',
      'seai_cron',
    );
  } else if (
    record.seaiApproval.status === 'approved' &&
    ...
    record = updatePostInstallTimeline(
      record,
      'seai_paid',
      '6-hour cron sync: SEAI released €22,100 via EFT',
      'seai_cron',
    );
  }
