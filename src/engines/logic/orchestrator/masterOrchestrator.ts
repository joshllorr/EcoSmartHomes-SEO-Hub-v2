...
export async function runOrchestrator(env: any): Promise<OrchestratorState> {
  ...
  const sampleUserIds =
    homeownersKeys.length > 0
      ? homeownersKeys
          .map((k) => k.name.replace('timeline_', ''))
  ...
  for (const user_id of sampleUserIds) {
    ...
    if (env.JOURNEY_TIMELINE) {
      try {
        const timeline = await env.JOURNEY_TIMELINE.get(`timeline_${user_id}`, {
          type: 'json',
        });
        const contractor_id =
          timeline?.events?.find(
            (ev: any) => ev.event === 'contractor_assigned',
          )?.phaseRef || 'ctr_2026_08_03_1612';
        if (contractor_id) {
