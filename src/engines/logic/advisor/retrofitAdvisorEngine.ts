...
export async function generateAdvisorReply(env: any, user_id: string, userMessage: string): Promise<string> {
  ...
  if (query.includes("next step") || query.includes("what next") || query.includes("status")) {
    const events = timeline.events || [];
    const lastEvent = events.length > 0 ? events[events.length - 1].event : "grant_eligibility_complete";
    const topUpgrade = upgrades?.recommendations?.[0]?.title || "Solar PV & Battery Upgrade";
  ...
  if (query.includes("contractor") || query.includes("builder") || query.includes("installer")) {
    const contractorEvent = timeline.events?.find(e => e.event === "contractor_assigned");
  ...
  if (query.includes("saving") || query.includes("cost") || query.includes("euro") || query.includes("bill") || query.includes("money")) {
    const topRec = upgrades?.recommendations?.[0];
    const recSavings = topRec?.estimatedSavings ? `€${topRec.estimatedSavings}/year` : `€${insights.avgAnnualSavings}/year`;
  ...
  if (query.includes("carbon") || query.includes("co2") || query.includes("environment") || query.includes("green")) {
    const totalCo2Kg = upgrades?.recommendations?.reduce((sum, r) => sum + (r.co2ImpactKgPerYear || 0), 0) || 1680;
  ...
  if (query.includes("forecast") || query.includes("timeline") || query.includes("delay") || query.includes("queue")) {
