...
import { Sparkles, RefreshCw, Compass, ShieldCheck, Heart, PartyPopper, CheckCircle2, Clock } from "lucide-react";
import { apiGet, apiPost } from "../../hooks/useApi";
import { CoachMessage } from "../../logic/coach/retrofitCoachEngine";

export default function RetrofitCoachView({ userId = "user_2026_08_03_1412" }: { userId?: string }) {
    ...
    <div className="flex flex-col gap-5 text-left font-sans max-w-4xl mx-auto">
      ...
      <div className="p-6 bg-slate-900/80 border border-emerald-500/20 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 39 Behavioral Automation Layer</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">Your Proactive AI Retrofit Coach</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">Automated progress monitoring, stall prevention, and sentiment-adapted guidance.</p>
        </div>
      ...
      <div className="flex flex-col gap-3">
        {messages.map((m) => {
          ...
          const toneClass = toneStyles[m.tone] || "bg-slate-900 border-white/10 text-white";

          return (
            <div key={m.id} className={`p-5 border rounded-2xl flex items-start gap-4 transition ${toneClass}`}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm font-sans leading-relaxed text-slate-100 font-medium mt-1">{m.text}</p>
              </div>
            </div>
          );
        })}
