...
export default function HomeownerPortal() {
  ...
  const [documents, setDocuments] = useState(() => [
    {
      ...
      status: 'verified',
      uploadedAt: 1786200000000,
    },
    {
      docName: 'Recent Electricity Utility Bill',
      status: 'verified',
      uploadedAt: 1786203600000,
    },
    {
  ...
  const handleUpload = async (docName: string) => {
    ...
    try {
      ...
      setDocuments((prev) =>
        prev.map((d) =>
          d.docName === docName
            ? { ...d, status: 'verified', uploadedAt: Date.now() }
            : d,
        ),
      );
    } catch (err) {
  ...
  const handleReschedule = async () => {
    const newDate = prompt(
      'Enter new preferred date (YYYY-MM-DD):',
      '2026-08-10',
    );
    if (!newDate) return;
    try {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">
                EcoSmartHomes Portal
              </h1>
              <p className="text-xs text-slate-400 font-mono">
                Sign in to manage your SEAI Grant Plan
              </p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 p-4 md:p-8 flex flex-col items-center justify-start font-sans text-left">
      <div className="w-full max-w-5xl flex flex-col gap-6">
        ...
        <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">
                Eircode:{' '}
                <strong className="text-emerald-300">{user.eircode}</strong> |
                Grant Plan:{' '}
                <strong className="text-emerald-300">{user.grant_id}</strong>
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-xs">
              ...
              <div className="p-4 bg-slate-900/80 border border-white/10 rounded-xl flex flex-col justify-between">
                ...
                <span className="text-2xl font-extrabold text-purple-400 mt-2">
                  High
                </span>
              </div>
            </div>
            ...
            <div className="p-6 bg-slate-900/80 border border-white/10 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <span className="text-[10px] font-mono text-emerald-400 font-bold uppercase">
                  Official SEAI Documentation
                </span>
                <h3 className="text-base font-bold text-white mt-0.5">
                  Download Your Custom Grant Plan PDF
                </h3>
                <p className="text-xs text-slate-400 font-mono mt-0.5">
                  Includes BER uplift breakdown, 5-year savings projection, and
                  surveyor sign-off.
                </p>
              </div>
        ...
        {activeTab === 'journey' && <JourneyView userId={user.user_id} />}
        ...
        {activeTab === 'coach' && <RetrofitCoachView userId={user.user_id} />}
              ...
              {[
                {
                  ...
                  amount: 8000,
                  desc: 'Fossil-fuel heating replacement grant.',
                },
                {
                  name: 'Cavity & Internal Wall Insulation',
                  amount: 4000,
                  desc: 'Thermal envelope insulation grant.',
                },
                {
                  name: 'Solar PV Panels',
                  amount: 3000,
                  desc: 'Rooftop zero-emissions micro-generation grant.',
                },
                {
                  name: 'Attic Insulation',
                  amount: 2000,
                  desc: 'High-retention ceiling insulation grant.',
                },
                {
                  name: 'Smart Heating Controls',
                  amount: 1000,
                  desc: 'Multi-zone thermostat controls grant.',
                },
                {
                  name: 'Full Retrofit + Combo Bonuses',
                  amount: 4100,
                  desc: 'Bonus for completing all measures + heat pump combo.',
                },
              ].map((g, idx) => (
                <div
                  key={idx}
        ...
        {activeTab === 'grants' && <GrantStatusView userId={user.user_id} />}
        ...
        {activeTab === 'timeline' && (
          <div className="p-6 bg-slate-900/80 border border-white/10 rounded-2xl flex flex-col gap-4 font-mono text-xs">
            ...
            <div className="flex flex-col gap-3">
              {[
                {
                  step: 'Step 1: Attic Insulation',
                  status: 'Completed',
                  date: 'August 2026',
                  grant: '€2,000',
                },
                {
                  step: 'Step 2: Smart Heating Controls',
                  status: 'Scheduled',
                  date: 'September 2026',
                  grant: '€700',
                },
                {
                  step: 'Step 3: Air-to-Water Heat Pump',
                  status: 'Pending',
                  date: 'October 2026',
                  grant: '€12,500',
                },
                {
                  step: 'Step 4: Solar PV Panels',
                  status: 'Pending',
                  date: 'November 2026',
                  grant: '€1,800',
                },
