...
import { Clock, ShieldCheck, CheckCircle2, ArrowRight, Award, Compass } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-5 relative pl-8 border-l-2 border-slate-700/60 font-mono text-xs my-4 text-left">
      {events.map((entry, idx) => {
        const meta = JOURNEY_EVENT_METADATA[entry.event as JourneyEventType] || {
          label: entry.event,
          icon: "📌",
          color: "#38bdf8",
          phase: "Phase 32"
        ...
        return (
          <div key={idx} className="relative flex flex-col gap-1.5 group">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-bold text-white text-sm" style={{ color: meta.color }}>
                {meta.label}
              </span>
              <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-slate-800 text-slate-300 border border-slate-700">
                {meta.phase}
              </span>
              <span className="text-slate-400 text-[11px] font-mono">{dateStr}</span>
            </div>
...
export default function JourneyView({ userId = "user_2026_08_03_1412" }: { userId?: string }) {
      ...
      <div className="p-6 bg-slate-900/80 border border-emerald-500/20 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 32 Master Journey Timeline</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">Your End-to-End Retrofit Journey</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">Timeline ID: <strong className="text-sky-300">{record.timeline_id}</strong></p>
        </div>

        <div className="flex items-center gap-3 font-mono">
          <div className="text-right">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Journey Completion</div>
