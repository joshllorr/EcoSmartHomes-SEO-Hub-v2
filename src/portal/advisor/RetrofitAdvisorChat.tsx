...
import { Send, Bot, User, Sparkles, RefreshCw, Compass, ShieldCheck, Euro, Leaf } from "lucide-react";
import { apiPost, apiGet } from "../../hooks/useApi";
import { AdvisorMessage } from "../../logic/advisor/retrofitAdvisorEngine";

export default function RetrofitAdvisorChat({ userId = "user_2026_08_03_1412" }: { userId?: string }) {
  ...
  const handleSend = async () => {
    ...
    setInput("");
    const userMsg: AdvisorMessage = { role: "user", text: userText, at: Date.now() };
    setMessages((prev) => [...prev, userMsg]);

    try {
      setSending(true);
      const res = await apiPost("/api/advisor/chat", { user_id: userId, message: userText });
      if (res && res.reply) {
        const assistantMsg: AdvisorMessage = { role: "assistant", text: res.reply, at: Date.now() };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 37 AI Retrofit Copilot</span>
            </div>
            <h2 className="text-lg font-bold text-white mt-0.5">Contextual Homeowner Guidance</h2>
          </div>
        </div>
          ...
          >
            <div
              className={`p-2 rounded-xl flex items-center justify-center h-8 w-8 shrink-0 ${
                m.role === "user" ? "bg-sky-500/20 text-sky-300 border border-sky-500/30" : "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30"
              }`}
            >
