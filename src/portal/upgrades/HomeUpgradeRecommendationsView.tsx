...
import { Sparkles, Zap, ShieldCheck, ArrowRight, Euro, Leaf, BatteryCharging, Flame, Layers, Clock } from "lucide-react";
import { apiGet, apiPost } from "../../hooks/useApi";
import { HomeUpgradeRecommendation } from "../../logic/upgrades/homeUpgradeEngine";

export default function HomeUpgradeRecommendationsView({ userId = "user_2026_08_03_1412" }: { userId?: string }) {
  ...
  const fetchRecommendations = async () => {
    try {
      setLoading(true);
      const res = await apiGet(`/api/upgrades/recommendations?user_id=${userId}`);
  ...
  const handleReGenerate = async () => {
    try {
      setGenerating(true);
      const res = await apiPost("/api/upgrades/recommendations/generate", { user_id: userId });
    ...
    <div className="flex flex-col gap-5 text-left font-sans">
      ...
      <div className="p-6 bg-slate-900/80 border border-emerald-500/20 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 34 AI Recommendation Engine</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">Next-Step Home Retrofit Upgrades</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">Journey-aware AI recommendations tailored to your property's installed measures.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {recs.map((r) => {
          ...
          const priorityColor = r.priority === "high" ? "bg-amber-500/20 text-amber-300 border-amber-500/30" : "bg-sky-500/20 text-sky-300 border-sky-500/30";

          return (
            <div key={r.id} className="p-5 bg-slate-900/80 border border-white/10 rounded-2xl flex flex-col justify-between gap-4 font-mono text-xs hover:border-emerald-500/40 transition">
              <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className={`px-2.5 py-0.5 rounded text-[10px] uppercase font-bold border ${priorityColor}`}>
                    {r.priority} Priority · {r.category}
                  </span>
                  <IconComp size={18} className="text-emerald-400" />
                </div>

                <h3 className="text-base font-bold text-white font-sans mt-1">{r.title}</h3>
                <p className="text-slate-300 text-xs font-sans leading-relaxed">{r.description}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-800/80">
                <div className="flex flex-col">
                  ...
                  <strong className="text-white text-sm">{r.estimatedCost ? `€${r.estimatedCost.toLocaleString()}` : "N/A"}</strong>
                </div>

                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 uppercase">Annual Savings</span>
                  <strong className="text-emerald-400 text-sm">{r.estimatedSavings ? `€${r.estimatedSavings}/yr` : "N/A"}</strong>
                </div>

                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 uppercase">CO₂ Saved</span>
                  <strong className="text-sky-300 text-sm">{r.co2ImpactKgPerYear ? `${r.co2ImpactKgPerYear} kg/yr` : "N/A"}</strong>
                </div>
              </div>
            </div>
          );
