...
export type AgentId =
  'heat-pumps' | 'solar' | 'insulation' | 'grants' | 'default';
...
const AGENT_LABELS: Record<
  AgentId,
  { label: string; icon: string; focus: string; lr: number }
> = {
  'heat-pumps': {
    label: 'Heat Pumps Agent',
    icon: '🔥',
    focus: 'CTR & High-Intent Conversions',
    lr: 0.05,
  },
  solar: {
    label: 'Solar PV Agent',
    icon: '☀️',
    focus: 'Backlink Growth & Impressions',
    lr: 0.06,
  },
  insulation: {
    label: 'Insulation Agent',
    icon: '🧱',
    focus: 'SERP Stability & Long-Tail Volume',
    lr: 0.04,
  },
  grants: {
    label: 'Grants Agent',
    icon: '💶',
    focus: 'CTR to SEAI Explainer Routes',
    lr: 0.03,
  },
  default: {
    label: 'Fleet Default Agent',
    icon: '🌐',
    focus: 'Balanced Autopilot Policy',
    lr: 0.05,
  },
};
...
export const RLPolicyDashboard: React.FC = () => {
  ...
  const [policies, setPolicies] = useState<Record<AgentId, PolicyState> | null>(
    null,
  );
  const [performances, setPerformances] = useState<Record<
    AgentId,
    AgentPerformance
  > | null>(null);
  ...
  const fetchRLData = async () => {
    try {
      const res = await fetch(
        `/api/rl/policy?agentId=${encodeURIComponent(selectedAgentId)}`,
      );
  ...
  const triggerRewardSim = async (agentTarget: AgentId) => {
    setLoading(true);
    setStatusMsg(
      `Simulating closed-loop outcome measurement for [Agent: ${agentTarget.toUpperCase()}]...`,
    );
    try {
      let payload = {};
      ...
      if (data.ok) {
        setStatusMsg(
          `Reward Calculated for [Agent: ${data.agentId}]: ${data.record.reward > 0 ? '+' : ''}${data.record.reward}! Agent policy updated.`,
        );
        fetchRLData();
      }
    } catch (err) {
  ...
  const triggerExperienceReplay = async () => {
    setLoading(true);
    setStatusMsg(
      `Executing Segmented Experience Replay for [Agent: ${selectedAgentId.toUpperCase()}]...`,
    );
    try {
      ...
      if (data.ok) {
        setStatusMsg(
          `Multi-Agent Experience Replay completed on ${data.replayCount} memories!`,
        );
        fetchRLData();
      }
    } catch (err) {
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Segmented RL Agents per Pillar & Domain: tuned learning rates,
            independent policy factor weights, per-agent experience replay, and
            smartness telemetry.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white font-mono">
              {AGENT_LABELS[selectedAgentId].label}
            </h3>
            <p className="text-xs text-slate-400 font-sans">
              Objective: {AGENT_LABELS[selectedAgentId].focus}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4 text-xs font-mono text-slate-300">
          <div>
            ...
            <span className="text-emerald-400 font-bold">
              {currentPolicy?.totalEvaluations ?? 0}
            </span>
          </div>
          <div>
            Learning Rate ($\alpha$):{' '}
            <span className="text-cyan-400 font-bold">
              {currentPolicy?.learningRate ?? 0.05}
            </span>
          </div>
          <div>
            Smartness Rank:{' '}
            <span className="text-amber-400 font-bold">
              {currentPerf?.smartnessLevel ?? 'Baseline'}
            </span>
          </div>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11px] text-emerald-400 font-bold mt-1">
                    {perf?.smartnessLevel || 'Novice Strategy'}
                  </p>
          ...
          <div className="space-y-3 font-mono text-xs">
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                ...
                <span className="text-emerald-400 font-bold">
                  {((currentPolicy?.ctrTrendWeight ?? 0.3) * 100).toFixed(1)}%
                </span>
              </div>
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((currentPolicy?.ctrTrendWeight ?? 0.3) / 0.95) * 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                ...
                <span className="text-cyan-400 font-bold">
                  {(
                    (currentPolicy?.serpVolatilityWeight ?? 0.25) * 100
                  ).toFixed(1)}
                  %
                </span>
              </div>
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((currentPolicy?.serpVolatilityWeight ?? 0.25) / 0.95) * 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                ...
                <span className="text-purple-400 font-bold">
                  {((currentPolicy?.backlinkGapWeight ?? 0.2) * 100).toFixed(1)}
                  %
                </span>
              </div>
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((currentPolicy?.backlinkGapWeight ?? 0.2) / 0.95) * 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                ...
                <span className="text-amber-400 font-bold">
                  {(
                    (currentPolicy?.contentVelocityWeight ?? 0.1) * 100
                  ).toFixed(1)}
                  %
                </span>
              </div>
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((currentPolicy?.contentVelocityWeight ?? 0.1) / 0.95) * 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                ...
                <span className="text-rose-400 font-bold">
                  {(
                    (currentPolicy?.pillarWeaknessWeight ?? 0.15) * 100
                  ).toFixed(1)}
                  %
                </span>
              </div>
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((currentPolicy?.pillarWeaknessWeight ?? 0.15) / 0.95) * 100)}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="md:col-span-5 bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-4 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-sans leading-tight mb-3">
              Trigger feedback cycles for individual Pillar Agents to test
              dedicated reward functions and per-agent policy adaptation:
            </p>
          </div>
      ...
      <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
        <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider flex items-center gap-2 font-mono">
          ...
          <span>
            MARL Segmented Memory Ledger ({experiences.length} Experiences)
          </span>
        </h3>

        {experiences.length === 0 ? (
          <p className="text-xs font-mono text-slate-500 italic">
            No experience records logged for this agent yet. Run a simulation
            above.
          </p>
        ) : (
            ...
            <table className="w-full text-xs font-mono text-left text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {experiences.slice(0, 8).map((e) => {
                  const ctrShift = (
                    (e.afterMetrics.ctr - e.beforeMetrics.ctr) *
                    100
                  ).toFixed(1);
                  const serpShift =
                    e.beforeMetrics.serpPosition - e.afterMetrics.serpPosition;
                  return (
                    <tr key={e.id} className="hover:bg-slate-900/50 transition">
                      ...
                      <td className="p-2 font-semibold text-white">
                        {e.siteId} /{' '}
                        <span className="text-slate-400">{e.slug}</span>
                      </td>
                      <td className="p-2">
                      ...
                      <td className="p-2">
                        <span
                          className={`font-bold ${e.reward > 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                        >
