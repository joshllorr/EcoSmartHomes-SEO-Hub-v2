...
export default function Writer() {
  const [localContent, setLocalContent] = useState(
    'Raising BER rating from G to A requires a strategic fabric-first retrofit sequence...',
  );
  ...
  const handleGenerateInternalLinks = async () => {
    setLoading(true);
    try {
      const pillar = pillarPages[0] || {
        title: 'BER Rating Ireland & Retrofit Master Guide',
      };
  ...
  return (
    <div
      className="p-6 space-y-6 text-left bg-white rounded-xl shadow-lg border border-slate-200"
      id="writer-component"
    >
      ...
      <div>
        <button
          ...
          disabled={loading}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-semibold text-sm transition cursor-pointer disabled:opacity-50 shadow-md"
        >
