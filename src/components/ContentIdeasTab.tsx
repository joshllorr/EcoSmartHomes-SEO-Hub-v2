...
const INITIAL_IDEAS: ContentIdea[] = [
  {
    ...
    summary: "How air moving through insulation can render 300mm of glass wool completely useless without airtight membranes.",
    tags: ["attic insulation", "seai attic insulation contractors", "seai attic insulation", "pumping insulation into walls"],
    monthlyVolume: "2.9K",
    oppScore: "435",
    difficulty: "HIGH",
    difficultyScore: 100,
  ...
  },
  {
    ...
    summary: "Analyzing the specific financial impact of the legislated 2026-2030 carbon tax increases on Irish households using fossil fuel heating.",
    tags: ["home heating oil prices", "home oil prices", "home fuel oil prices"],
    monthlyVolume: "6.6K",
    oppScore: "6.4K",
    difficulty: "LOW",
    difficultyScore: 4,
  ...
  },
  {
    ...
    summary: "An interactive quiz & master guide that filters the complex 2026 SEAI grant landscape (including the €12,500 heat pump grant and €2,000 attic grants) to tell users exactly what they can claim.",
  ...
  },
  {
    ...
    type: "Trending",
    age: "6d ago",
    title: "Beyond the Smoke: Why Indoor Air Quality is the Next Frontier in Irish Home Retrofitting",
    summary: "Explains how modern retrofits, specifically MVHR (Mechanical Ventilation with Heat Recovery), protect homes from outdoor pollutants, hay fever, and smoke while maintaining thermal efficiency.",
    tags: ["why is the air quality so bad today", "mechanical ventilation heat recovery ireland", "indoor air quality sensors"],
    monthlyVolume: "2.4K",
    oppScore: "360",
    difficulty: "HIGH",
    difficultyScore: 100,
  ...
  },
  {
    ...
    summary: "Identifying energy loss points and preventing moisture issues during the retrofit journey.",
    tags: ["air tightness test", "building air tightness test", "airtightness test"],
    monthlyVolume: "640",
    oppScore: "144",
    difficulty: "MEDIUM",
    difficultyScore: 42,
  ...
  },
  {
    ...
    summary: "Diagnosing common control and flow issues that lead to equipment wear and inefficiency.",
  ...
  },
  {
    ...
    summary: "An interactive tool that allows Irish homeowners to input their current fuel usage (Kerosene, Gas, or Diesel) and see exactly how legislated carbon tax increases (reaching €100/tonne by 2030) will impact their annual bills.",
    tags: ["carbon tax ireland", "carbon tax calculator ireland", "carbon tax increase ireland"],
    monthlyVolume: "590",
    oppScore: "590",
    difficulty: "LOW",
    difficultyScore: 0,
...
export default function ContentIdeasTab({ onOpenInWriter, site }: ContentIdeasTabProps) {
  const [ideas, setIdeas] = useState<ContentIdea[]>(INITIAL_IDEAS);
  const [targetSite, setTargetSite] = useState<string>(site || "ecosmarthomes.ie");
  const [selectedCategory, setSelectedCategory] = useState<string>("All");
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [aiGuidance, setAiGuidance] = useState<string>("home retrofitting, BER ratings, SEAI grants, insulation, heat pumps");
  const [isGenerating, setIsGenerating] = useState<boolean>(false);
  const [selectedModalIdea, setSelectedModalIdea] = useState<ContentIdea | null>(null);
  
  // Sorting & Quick Filter states
  const [sortBy, setSortBy] = useState<"oppScore" | "monthlyVolume" | "difficulty" | "fresh">("oppScore");
  const [quickFilter, setQuickFilter] = useState<"all" | "quickWins" | "risingTrends" | "bookmarked">("all");
  ...
  const filteredIdeas = useMemo(() => {
    return ideas.filter((idea) => {
      const categoryMatch = selectedCategory === "All" || idea.type === selectedCategory;
      ...
      if (sortBy === "monthlyVolume") {
        const numA = parseFloat(a.monthlyVolume.replace(/[^0-9.]/g, "")) * (a.monthlyVolume.includes("K") ? 1000 : 1);
        const numB = parseFloat(b.monthlyVolume.replace(/[^0-9.]/g, "")) * (b.monthlyVolume.includes("K") ? 1000 : 1);
        return numB - numA;
      }
      if (sortBy === "difficulty") {
        const diffA = a.difficultyScore ?? (a.difficulty === "LOW" ? 10 : a.difficulty === "MEDIUM" ? 50 : 90);
        const diffB = b.difficultyScore ?? (b.difficulty === "LOW" ? 10 : b.difficulty === "MEDIUM" ? 50 : 90);
      ...
      // Default: oppScore
      const oppA = parseFloat(a.oppScore.replace(/[^0-9.]/g, "")) * (a.oppScore.includes("K") ? 1000 : 1);
      const oppB = parseFloat(b.oppScore.replace(/[^0-9.]/g, "")) * (b.oppScore.includes("K") ? 1000 : 1);
      return oppB - oppA;
    });
  }, [ideas, selectedCategory, searchQuery, quickFilter, bookmarkedIds, sortBy]);
      ...
      if (res.ok) {
        ...
        if (data.ideas && data.ideas.length > 0) {
          setIdeas((prev) => {
            ...
            const newFresh = data.ideas.filter((i: ContentIdea) => !existingTitles.has(i.title.toLowerCase()));
          ...
          setNotification(`Discovered ${data.ideas.length} fresh content opportunities for ${targetSite || site} using Google Search AI!`);
  ...
  const handleExportCSV = () => {
    const headers = ["Title", "Type", "Target Query", "Monthly Volume", "Opportunity Score", "Difficulty", "CPC Range", "Trend", "Tags", "Summary"];
  ...
  const getDifficultyBadge = (diff: "LOW" | "MEDIUM" | "HIGH", score?: number) => {
    if (diff === "LOW") {
      return (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <span className="text-xs font-mono text-slate-400">{ideas.length} Content Opportunities Identified</span>
          </div>
          ...
          <p className="text-slate-400 text-xs mt-1">
            Uncover competitor content gaps, breakout search trends, and topical clusters tailored for <strong className="text-slate-200 font-mono">{targetSite}</strong>.
          </p>
        </div>
        ...
        <div className="bg-black/40 border border-white/10 rounded-xl p-2.5 flex items-center gap-2 self-start md:self-auto">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/20 text-[#34d399] border border-emerald-500/30 rounded-lg text-xs font-bold font-mono">
            <span>Step 1</span>
            <span className="text-[10px] uppercase font-semibold text-emerald-300">Discover</span>
          </div>
      ...
      <div className="bg-gradient-to-r from-slate-900/95 via-slate-900/90 to-slate-900/95 border border-white/10 hover:border-emerald-500/30 rounded-2xl p-4 sm:p-5 shadow-xl space-y-4 transition">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div className="md:col-span-4 space-y-1">
            <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Target Site / Brand</span>
          ...
          <div className="md:col-span-5 space-y-1">
            <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Focus Topics & Niche Keywords</span>
            <div className="relative">
              <Lightbulb size={14} className="absolute left-3 top-3 text-slate-500" />
              <input
                type="text"
                value={aiGuidance}
        ...
        {isGrounded && (
          <div className="bg-black/40 border border-emerald-500/20 rounded-xl p-3 space-y-2 text-xs font-mono">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 pb-1.5">
              ...
              <span className="text-[10px] text-slate-400">{groundingQueries.length} Search queries executed</span>
            </div>

            <div className="flex flex-wrap gap-1.5">
              {groundingQueries.map((q, idx) => (
                <span key={idx} className="bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 text-[10px] px-2 py-0.5 rounded-md flex items-center gap-1">
                  <Search size={10} />
                  <span>{q}</span>
                </span>
              ))}
            ...
            {groundingSources.length > 0 && (
              <div className="pt-1 flex flex-wrap items-center gap-2 text-[10px] text-slate-400">
                <span className="text-slate-500 font-semibold">Live Sources:</span>
        ...
        <div className="bg-gradient-to-br from-emerald-950/40 via-slate-900/90 to-slate-900/90 border border-emerald-500/30 rounded-2xl p-5 sm:p-6 shadow-2xl relative overflow-hidden space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-slate-400">Priority Opportunity #1</span>
            </div>
            <button
            ...
            >
              {bookmarkedIds.has(ideas[0].id) ? (
                <BookmarkCheck size={16} className="text-amber-400 fill-amber-400/20" />
              ) : (
                <Bookmark size={16} />
              )}
            </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
            <div className="lg:col-span-8 space-y-3">
              <h3 className="text-lg sm:text-xl font-display font-bold text-white tracking-tight hover:text-[#34d399] transition cursor-pointer" onClick={() => setSelectedModalIdea(ideas[0])}>
                {ideas[0].title}
              </h3>
              ...
              <div className="flex flex-wrap items-center gap-4 pt-1 font-mono text-xs">
                <div className="flex items-center gap-1.5 bg-black/40 px-3 py-1.5 rounded-lg border border-white/10">
                  ...
                  <strong className="text-white font-bold">{ideas[0].monthlyVolume}</strong>
                ...
                <div className="flex items-center gap-1.5 bg-black/40 px-3 py-1.5 rounded-lg border border-white/10">
                  ...
                  <strong className="text-sky-300 font-bold">{ideas[0].oppScore}</strong>
                </div>

                <div className="flex items-center gap-1.5 bg-black/40 px-3 py-1.5 rounded-lg border border-white/10">
                  <span className="text-slate-400">Difficulty:</span>
                  {getDifficultyBadge(ideas[0].difficulty, ideas[0].difficultyScore)}
                </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1 text-xs">
                <span className="font-mono text-slate-400">
                  targets <strong className="text-white bg-white/10 px-2 py-0.5 rounded font-mono">“{ideas[0].targetQuery}”</strong>
                </span>
            ...
            <div className="lg:col-span-4 bg-black/50 border border-white/10 rounded-xl p-4 space-y-2 font-mono text-xs">
              ...
              <div className="space-y-1.5 divide-y divide-white/5">
                {ideas[0].subtopics.map((sub, i) => (
                  <div key={i} className="flex items-center justify-between pt-1 text-[11px]">
                    <span className="text-slate-300 truncate max-w-[170px]">{sub.name}</span>
      ...
      <div className="space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-slate-900 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-300 font-mono">
              ...
              >
                <option value="oppScore" className="bg-slate-900 text-white">Sort: Opp Score</option>
                <option value="monthlyVolume" className="bg-slate-900 text-white">Sort: Search Volume</option>
                <option value="difficulty" className="bg-slate-900 text-white">Sort: Quick Wins (Low Diff)</option>
                <option value="fresh" className="bg-slate-900 text-white">Sort: Fresh First</option>
              </select>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none pb-1">
            {(["All", "Content Gaps", "Topics", "Trending", "Pillar Pages"] as const).map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
                    isActive ? "bg-black/20 text-[#0f172a]" : "bg-black/30 text-slate-400"
                  }`}>
                    {count}
                  </span>
                </button>
          ...
          <div className="flex items-center gap-1.5 font-mono text-xs">
            <button
              onClick={() => setQuickFilter(quickFilter === "quickWins" ? "all" : "quickWins")}
            ...
            <button
              onClick={() => setQuickFilter(quickFilter === "risingTrends" ? "all" : "risingTrends")}
              className={`px-2.5 py-1 rounded-lg border text-[10px] font-bold transition cursor-pointer flex items-center gap-1 ${
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-[11px] font-mono">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {isBookmarked ? (
                        <BookmarkCheck size={14} className="text-amber-400 fill-amber-400/20" />
                      ) : (
                        <Bookmark size={14} />
                      )}
                    </button>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {idea.tags.slice(0, 3).map((tag, i) => (
                    <span key={i} className="bg-black/30 border border-white/5 text-slate-300 text-[10px] font-mono px-2 py-0.5 rounded truncate max-w-[150px]">
                      {tag}
                    </span>
                  ))}
                  {idea.tags.length > 3 && (
                    <span className="text-[10px] font-mono text-slate-500">+{idea.tags.length - 3}</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3 pt-3 border-t border-white/10 font-mono text-xs">
                <div className="flex items-center justify-between text-slate-300">
                  <div className="flex items-center gap-1">
                    <BarChart3 size={13} className="text-[#34d399]" />
                    <span className="font-bold text-white">{idea.monthlyVolume}</span>
                    <span className="text-[10px] text-slate-500">/mo</span>
                  </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 border-t border-white/5">
                  ...
                  <div className="flex items-center gap-1 text-emerald-400">
                    <span>↗ rising</span>
                    {idea.peakMonth && <span className="text-slate-500">· peaks {idea.peakMonth}</span>}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-1 text-[11px]">
                  <span className="text-slate-400">
                    targets <strong className="text-slate-200">“{idea.targetQuery}”</strong>
                  </span>
                  <span className={`font-bold flex items-center gap-1 ${
                    idea.demandStatus === "strong demand" ? "text-emerald-400" : "text-amber-300"
                  }`}>
                    {idea.demandStatus === "strong demand" ? "✅ strong demand" : "◑ niche"}
                  </span>
                </div>
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono text-slate-400">Targeting {targetSite}</span>
                </div>
              ...
              <div className="bg-black/60 border border-emerald-500/30 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                  ...
                  <span className="text-[10px] font-mono text-slate-400">Pillar & Spoke Internal Linking</span>
                </div>
                ...
                <div className="relative p-4 bg-slate-950/80 rounded-xl border border-white/5 flex flex-col items-center justify-center space-y-4">
                  ...
                  <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-mono text-xs font-bold px-4 py-2 rounded-xl shadow-lg border border-emerald-300/40 text-center max-w-sm">
                    <div className="text-[9px] uppercase tracking-wider text-emerald-200">Main Hub Page</div>
                    <div>{selectedModalIdea.targetQuery} ({selectedModalIdea.monthlyVolume}/mo)</div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 w-full font-mono text-[11px]">
                    {selectedModalIdea.subtopics.map((sub, i) => (
                      <div key={i} className="bg-slate-900 border border-emerald-500/20 hover:border-emerald-500/50 p-2.5 rounded-xl text-center space-y-1">
                        <span className="text-slate-300 font-semibold block truncate">{sub.name}</span>
                        <span className="text-[#34d399] font-bold text-[10px] block">{sub.vol} /mo</span>
                        <span className="text-[9px] text-slate-500 block border-t border-white/5 pt-1">Anchor: link to hub</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
                <div className="bg-black/50 p-3 rounded-xl border border-white/5 space-y-0.5">
                  <span className="text-[10px] text-slate-400">Monthly Vol</span>
                  <p className="text-base font-bold text-white">{selectedModalIdea.monthlyVolume}</p>
                </div>
                <div className="bg-black/50 p-3 rounded-xl border border-white/5 space-y-0.5">
                  <span className="text-[10px] text-slate-400">Opportunity</span>
                  <p className="text-base font-bold text-sky-300">{selectedModalIdea.oppScore}</p>
                </div>
                <div className="bg-black/50 p-3 rounded-xl border border-white/5 space-y-0.5">
                  <span className="text-[10px] text-slate-400">CPC Est.</span>
                  <p className="text-xs font-bold text-amber-300 mt-1">{selectedModalIdea.cpcRange || "N/A"}</p>
                </div>
                <div className="bg-black/50 p-3 rounded-xl border border-white/5 space-y-0.5">
                  <span className="text-[10px] text-slate-400">Difficulty</span>
                  <div className="mt-1">{getDifficultyBadge(selectedModalIdea.difficulty, selectedModalIdea.difficultyScore)}</div>
                </div>
              </div>

              {/* Keywords Tag List */}
              <div className="space-y-2">
                <h4 className="text-xs font-mono font-bold text-slate-200 uppercase tracking-wider">Target Long-Tail Keywords:</h4>
                <div className="flex flex-wrap gap-1.5 font-mono text-xs">
                  {selectedModalIdea.tags.map((t, idx) => (
                    <span key={idx} className="bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg text-slate-200">
                      #{t}
                    </span>
                  ))}
                </div>
