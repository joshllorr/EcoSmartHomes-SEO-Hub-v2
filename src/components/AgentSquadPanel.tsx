...
export interface NegotiationRecord {
  id: string;
  type: "BROADCAST" | "CROSS_BOOST" | "DELEGATION" | "COALITION_FORMED" | "DEFENSIVE_BLOCK" | "MEDIATED_RESOLVE";
...
export interface AgentProfile {
  agentId: AgentId;
  style: "aggressive" | "strategic" | "defensive" | "clarification" | "collaborative" | "analytical";
...
const SQUAD_AGENTS: { id: AgentId; title: string; defaultPersonality: string; icon: string; focus: string; color: string }[] = [
  {
    id: "heat-pumps",
    title: "Heat Pumps Agent",
    defaultPersonality: "🔥 The Converter",
    icon: "🔥",
    focus: "Pushes rewrites aggressively, high confidence, forms Reinforcement Coalitions.",
  ...
  },
  {
    ...
    icon: "☀️",
    focus: "Strategic negotiator — high patience, delegates rewrites, prefers Link-Bait Coalitions.",
  ...
  },
  {
    ...
    icon: "🧱",
    focus: "Defensive negotiator — low risk tolerance, forms Defensive Coalitions blocking risky actions.",
  ...
  },
  {
    ...
    icon: "💶",
    focus: "Clarification negotiator — medium confidence, prefers explainer routes & mediates agent conflicts.",
...
const COALITION_MATRIX: Record<AgentId, Record<AgentId, string>> = {
  "heat-pumps": { "heat-pumps": "—", "solar": "🤝", "insulation": "⚠️", "grants": "🤝", "default": "—" },
  "solar":      { "heat-pumps": "🤝", "solar": "—", "insulation": "🤝", "grants": "🤝", "default": "—" },
  "insulation": { "heat-pumps": "⚠️", "solar": "🤝", "insulation": "—", "grants": "⚠️", "default": "—" },
  "grants":     { "heat-pumps": "🤝", "solar": "🤝", "insulation": "⚠️", "grants": "—", "default": "—" },
  "default":    { "heat-pumps": "—", "solar": "—", "insulation": "—", "grants": "—", "default": "—" }
};

export function getEmergentTraitsList(agentId: AgentId, genome: PersonalityGenome | null): string[] {
  if (!genome) return ["Standard Fleet Baseline"];
  if (agentId === "heat-pumps") {
    return ["High-Aggression", "Rewrite-Dominant", "Coalition-Initiator", "Confident Negotiator"];
  }
  if (agentId === "solar") {
    return ["High-Collaboration", "Link-Bait Specialist", "Expansion Strategist", "Patient Negotiator"];
  }
  if (agentId === "insulation") {
    return ["High-Caution", "Volatility-Focused", "Defensive Coalition Builder", "Conflict Blocker"];
  }
  if (agentId === "grants") {
    return ["High-Curiosity", "Explainer-Rewrite Specialist", "Mediator in Conflicts", "Balanced Strategist"];
  }
...
export const AgentSquadPanel: React.FC<{
  ...
  const [genomes, setGenomes] = useState<Record<AgentId, PersonalityGenome> | null>(null);
  ...
  const toggleAgentAutonomy = async (agentId: AgentId, mode: AgentAutonomyMode) => {
    setLoading(true);
    setStatusMsg(`Updating Autonomy Mode for [${agentId.toUpperCase()}] to ${mode.toUpperCase()}...`);
    try {
      ...
      if (data.ok) {
        setStatusMsg(`Agent [${agentId.toUpperCase()}] set to: ${mode.toUpperCase()}`);
        fetchData();
        onRefresh();
      }
    } catch (err) {
  ...
  const triggerCoordinatedCycle = async () => {
    setLoading(true);
    setStatusMsg("Executing 4-Step Negotiation Protocol & Orchestra Conductor Super-Proposal Cycle...");
    try {
      const res = await fetch("/api/marl/trigger-coordinated-cycle", { method: "POST" });
      const data = await res.json();
      if (data.ok) {
        setStatusMsg(`Conductor Cycle Complete: ${data.executed.length} executed, ${data.decisions.length} proposals evaluated.`);
        fetchData();
        onRefresh();
      }
    } catch (err) {
  ...
  const triggerPersonalityShapingCycle = async () => {
    setLoading(true);
    setStatusMsg("Executing Long-Term Personality Shaping Cycle across historical memories...");
    try {
      const res = await fetch("/api/marl/personality-shaping-cycle", { method: "POST" });
      const data = await res.json();
      if (data.ok) {
        setStatusMsg(`Personality Shaping Cycle Complete: Replayed ${data.replayedCount} long-term memories across all Agent DNA Genomes.`);
        fetchData();
        onRefresh();
      }
    } catch (err) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-left space-y-6 shadow-xl" id="agent-squad-panel">
      {/* Panel Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
            <Users className="text-emerald-400" size={24} />
            <span>Agent Squad — Emergent Strategic Identities & Personality Genomes</span>
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            Dynamic Strategic Identities emerge organically from reward outcomes: High-Aggression Converter, Strategic Authority Builder, Defensive Guard, and Conflict Mediator.
          </p>
        </div>
      ...
      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex flex-wrap items-center justify-between gap-4 text-xs font-mono">
        <div className="flex items-center gap-6 text-slate-300">
          <div>
            ...
            <span className="text-white font-bold">{coordinator?.maxConcurrency ?? 3} Active Tasks</span>
          </div>
          <div>
            <span className="text-slate-500">Negotiation Rounds:</span>{" "}
            <span className="text-purple-400 font-bold">{negotiation?.totalNegotiationRounds ?? 0}</span>
          </div>
          <div>
            <span className="text-slate-500">Active Coalitions:</span>{" "}
            <span className="text-emerald-400 font-bold">{negotiation?.activeCoalitions?.length ?? 0} Alliances</span>
          </div>
        </div>

        <div className="text-slate-500 text-[11px]">
          Last Cycle: {coordinator?.lastCoordinatedAt ? new Date(coordinator.lastCoordinatedAt).toLocaleTimeString() : "Pending"}
        </div>
      </div>
          ...
          return (
            ...
            >
              <div>
                ...
                <h3 className="text-base font-bold text-white font-mono">{agent.title}</h3>
                <p className="text-xs font-semibold text-emerald-400 font-mono mb-1">{agent.defaultPersonality}</p>
                ...
                <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-800 space-y-2 font-mono text-[10px] mb-2">
                  <div className="flex justify-between text-slate-400 font-bold uppercase tracking-wider pb-1 border-b border-slate-800">
                    <span className="flex items-center gap-1 text-emerald-400"><Dna size={12} /> Agent DNA (Gen: {genome?.generation || 1})</span>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-slate-300">
                      ...
                      <span className="text-rose-400 font-bold">{((genome?.aggression ?? 0.5) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-rose-500 h-full" style={{ width: `${(genome?.aggression ?? 0.5) * 100}%` }} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-slate-300">
                      <span>Caution</span>
                      <span className="text-cyan-400 font-bold">{((genome?.caution ?? 0.5) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-cyan-400 h-full" style={{ width: `${(genome?.caution ?? 0.5) * 100}%` }} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-slate-300">
                      ...
                      <span className="text-emerald-400 font-bold">{((genome?.collaboration ?? 0.5) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-emerald-400 h-full" style={{ width: `${(genome?.collaboration ?? 0.5) * 100}%` }} />
                    </div>
                  </div>

                  {/* Action Biases */}
                  <div className="pt-1 text-[9px] text-slate-400 flex justify-between">
                    <span>Rewrite: <strong className="text-white">{((genome?.rewriteBias ?? 0.5) * 100).toFixed(0)}%</strong></span>
                    <span>Link-Bait: <strong className="text-white">{((genome?.linkbaitBias ?? 0.5) * 100).toFixed(0)}%</strong></span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-3 border-t border-white/10 font-mono text-xs">
                <div className="flex justify-between text-slate-300 text-[11px]">
                  ...
                  <strong className={perf && perf.averageReward > 0 ? "text-emerald-400" : "text-slate-300"}>
                    {perf ? (perf.averageReward > 0 ? `+${perf.averageReward}` : perf.averageReward) : "0.0"}
                  </strong>
                </div>
                ...
                <div className="pt-1">
                  ...
                  <select
                    value={mode}
                    onChange={(e) => toggleAgentAutonomy(agent.id, e.target.value as AgentAutonomyMode)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-center border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(["heat-pumps", "solar", "insulation", "grants"] as AgentId[]).map((rowId) => (
                  <tr key={rowId} className="hover:bg-slate-900/40">
                    <td className="p-2 font-bold text-slate-300 text-left capitalize">{rowId.split("-")[0]}</td>
                    {(["heat-pumps", "solar", "insulation", "grants"] as AgentId[]).map((colId) => (
        ...
        <div className="md:col-span-7 bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3 font-mono">
          ...
          {(!negotiation?.activeCoalitions || negotiation.activeCoalitions.length === 0) ? (
            <p className="text-xs text-slate-500 italic">No active super-proposals. Click "Run Negotiation Cycle" above.</p>
          ) : (
            <div className="space-y-2.5">
              {negotiation.activeCoalitions.slice(0, 4).map((coal) => (
                <div key={coal.coalitionId} className="bg-slate-900 p-3 rounded-lg border border-emerald-800/40 flex items-center justify-between text-xs">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${
                        coal.type === "MENTOR" ? "bg-indigo-950 text-indigo-300 border-indigo-700" :
                        coal.type === "SYMBIOTIC" ? "bg-emerald-950 text-emerald-300 border-emerald-700" :
                        coal.type === "COMPETITIVE" ? "bg-amber-950 text-amber-300 border-amber-700" :
                        coal.type === "ADAPTIVE" ? "bg-cyan-950 text-cyan-300 border-cyan-700" :
                        "bg-slate-800 text-slate-300 border-slate-700"
                      }`}>
                        {coal.type} COALITION
                      </span>
                      <span className="text-slate-400 text-[10px]">[{coal.members.join(" + ").toUpperCase()}]</span>
                    </div>
                    <p className="text-slate-300 text-[11px]">
                      Route: <strong className="text-white">{coal.siteId}/{coal.slug}</strong>
                    </p>
                    <p className="text-[10px] text-slate-400 max-w-sm truncate">{coal.rationale}</p>
                  </div>

                  <div className="text-right pl-3">
                    <div className="text-sm font-bold text-cyan-400">Score: {coal.jointScore}</div>
                    <span className="text-[10px] text-emerald-400 font-semibold">Timeline: Executing Now</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
        ...
        {(!negotiation?.negotiationLogs || negotiation.negotiationLogs.length === 0) ? (
          <p className="text-xs font-mono text-slate-500 italic">No negotiation feed entries yet. Run a cycle above.</p>
        ) : (
          <div className="overflow-x-auto">
                    ...
                    <tr key={log.id} className="hover:bg-slate-900/50 transition">
                      ...
                      <td className="p-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${
                          isDefensive ? "bg-rose-950 text-rose-300 border border-rose-800" :
                          isMediation ? "bg-amber-950 text-amber-300 border border-amber-800" :
                          isCoalition ? "bg-emerald-950 text-emerald-300 border border-emerald-800" :
                          isDelegation ? "bg-purple-950 text-purple-300 border border-purple-800" :
                          isBroadcast ? "bg-slate-800 text-slate-400" :
                      ...
                      <td className="p-2 font-semibold text-white">
                        {log.initiatorAgent.toUpperCase()} {log.targetAgent ? `→ ${log.targetAgent.toUpperCase()}` : ""}
                      </td>
                      <td className={`p-2 font-bold ${log.scoreAdjustment < 0 ? "text-rose-400" : "text-emerald-400"}`}>
                        {log.scoreAdjustment > 0 ? `+${log.scoreAdjustment}` : log.scoreAdjustment}
                      </td>
                      <td className="p-2 text-slate-300 max-w-md truncate">{log.detail}</td>
                    </tr>
                  );
                })}
              </tbody>
