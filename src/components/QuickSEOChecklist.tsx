...
const DEFAULT_CHECKLIST: ChecklistItem[] = [
  {
    ...
    category: "On-Page",
    description: "Ensure all core landing pages have 150-160 character meta descriptions targeting high-intent Irish retrofit terms.",
    xp: 15,
    impact: "High"
  },
  {
    ...
    category: "On-Page",
    description: "Include primary target terms (e.g. 'BER Rating Ireland') in a single H1 header per page without keyword stuffing.",
    xp: 15,
    impact: "High"
  },
  {
    ...
    description: "Embed Organization, WebSite, and FAQPage schemas so search engines & LLMs cite your site authoritatively.",
    xp: 20,
    impact: "Critical"
  },
  {
    ...
    category: "On-Page",
    description: "Describe diagrams, heat pump photos, and retrofitting infographics using descriptive alt attributes.",
    xp: 10,
    impact: "Medium"
  },
  {
    ...
    description: "Format key answers into clear Question/Answer pairs that Perplexity, ChatGPT, and Gemini cite directly.",
    xp: 25,
    impact: "Critical"
  },
  {
    ...
    category: "Technical & Schema",
    description: "Prevent duplicate content issues caused by tracking query parameters or sitemap variations.",
    xp: 15,
    impact: "Medium"
  },
  {
    ...
    category: "Technical & Schema",
    description: "Compress hero images and defer non-critical scripts to maintain a sub-2.5s Largest Contentful Paint.",
    xp: 20,
    impact: "High"
  },
  {
    ...
    category: "AI Visibility",
    description: "Link energy calculators and grant guides together using keyword-rich anchor text to build topical authority.",
    xp: 15,
    impact: "High"
  }
];
...
export default function QuickSEOChecklist({
  ...
  const [recentXpToast, setRecentXpToast] = useState<{ amount: number; title: string } | null>(null);

  useEffect(() => {
    try {
      localStorage.setItem("ecosmart_seo_checklist_v1", JSON.stringify(completedIds));
    } catch (e) {
  ...
  const earnedXP = DEFAULT_CHECKLIST
    .filter((item) => completedIds.includes(item.id))
    .reduce((acc, curr) => acc + curr.xp, 0);

  const completedCount = completedIds.length;
  const progressPercent = Math.round((completedCount / DEFAULT_CHECKLIST.length) * 100);
  ...
  return (
    <div className="glass-card p-6 space-y-5 text-left relative overflow-hidden" id="quick-seo-checklist-component">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-white text-base tracking-tight">Quick SEO Checklist</h3>
          ...
          <p className="text-xs text-slate-400">
            Check off key optimization items for <span className="text-slate-200 font-medium font-mono">{targetDomain}</span> to boost rank and earn XP.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-black/30 border border-white/10 p-2.5 rounded-xl shrink-0">
          ...
          <div>
            <div className="text-[10px] uppercase font-mono text-slate-400 font-semibold">Checklist XP</div>
      ...
      <div className="space-y-1.5 bg-black/20 border border-white/5 p-3 rounded-xl">
        <div className="flex justify-between items-center text-xs">
          <span className="font-semibold text-slate-300">Overall Optimization Progress</span>
          <span className="font-mono text-[#34d399] font-bold">{completedCount} of {DEFAULT_CHECKLIST.length} done ({progressPercent}%)</span>
        </div>
        ...
        {filteredItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              >
                {isDone ? (
                  <CheckCircle2 size={18} className="text-[#34d399] fill-emerald-500/20" />
                ) : (
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    >
                      <Zap size={11} className={isDone ? "fill-[#34d399]" : "fill-indigo-300"} />
