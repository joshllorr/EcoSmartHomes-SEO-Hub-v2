...
export default function EntityCardPreview({
  ...
  const [activeTab, setActiveTab] = useState<"knowledge_panel" | "ai_perception" | "graph_nodes">("knowledge_panel");

  // Helper for Schema.org property hover tooltips
  const PropertyTooltip = ({ propName, schemaType, description }: { propName: string; schemaType?: string; description: string }) => (
    <div className="opacity-0 group-hover/prop:opacity-100 transition-all duration-200 pointer-events-none absolute z-30 bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-64 p-2.5 bg-slate-900/95 text-slate-200 text-[10px] font-mono leading-tight rounded-xl border border-emerald-500/40 shadow-2xl backdrop-blur-md space-y-1">
      <div className="flex items-center justify-between text-emerald-400 font-bold border-b border-white/10 pb-1">
        ...
        {schemaType && <span className="text-[9px] bg-emerald-500/20 px-1.5 py-0.2 rounded text-emerald-300 font-semibold">{schemaType}</span>}
      </div>
      <p className="text-slate-300 font-sans text-[10px] leading-relaxed">{description}</p>
    </div>
  );
  ...
  const orgNode = graphNodes.find(n => n["@type"] === "Organization" || n["@type"] === "LocalBusiness" || n["@type"]?.includes?.("Business"));
  const websiteNode = graphNodes.find(n => n["@type"] === "WebSite");
  const faqNode = graphNodes.find(n => n["@type"] === "FAQPage");
  const searchActionNode = graphNodes.find(n => n["@type"] === "SearchAction" || n?.potentialAction?.["@type"] === "SearchAction");

  // Fallbacks
  const displayTitle = orgName || orgNode?.name || websiteNode?.name || "EcoSmart Homes";
  const displayUrl = targetUrl || orgNode?.url || websiteNode?.url || "https://ecosmarthomes.ie";
  const cleanDomain = displayUrl.replace(/^https?:\/\//i, "").replace(/\/.*$/, "");
  const displayDesc = description || websiteNode?.description || "Energy efficiency, home retrofitting, and BER rating optimization authority.";
  ...
  const faqQuestions = faqNode?.mainEntity || [
    {
      ...
      acceptedAnswer: { text: "Upgrading a BER rating involves insulation improvements, draft proofing, heat pump integration, and solar PV installations." }
    },
    {
      name: "What SEAI grants are available for home retrofitting?",
      acceptedAnswer: { text: "SEAI offers grants for wall insulation, attic insulation, heat pumps, solar water heating, and solar PV panels." }
    }
  ];
  ...
  const urlStatus: FieldStatus = (() => {
    if (!displayUrl || displayUrl === "https://" || displayUrl === "http://") {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "Target URL is blank or incomplete." };
    }
    if (!displayUrl.startsWith("http://") && !displayUrl.startsWith("https://")) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "URL missing http:// or https:// protocol prefix." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: "Canonical URL and domain URI validated." };
  })();

  const nameStatus: FieldStatus = (() => {
    if (!displayTitle || displayTitle.trim().length < 2) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "Organization name is missing or too short." };
    }
    if (!orgNode && !websiteNode) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "No Organization or WebSite type node in graph." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: "Primary entity name & @type classification defined." };
  })();

  const descStatus: FieldStatus = (() => {
    const len = displayDesc ? displayDesc.trim().length : 0;
    if (len === 0) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "Description property is missing." };
    }
    if (len < 30) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: `Description short (${len} chars). Recommend at least 30 chars.` };
    }
    if (len > 320) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: `Description long (${len} chars). May truncate in Google snippets.` };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: `Description length (${len} chars) optimal for SERPs & AI engines.` };
  })();

  const areaStatus: FieldStatus = (() => {
    if (!selectedAreas || selectedAreas.length === 0) {
      return { isValid: false, status: "warning", label: "Broad Coverage", reason: "No specific local target areas selected. Set regional suburbs in generator." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: `${selectedAreas.length} local target territories mapped in areaServed.` };
  })();

  const topicStatus: FieldStatus = (() => {
    if (!knowsAboutList || knowsAboutList.length < 3) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "Fewer than 3 topic entities declared in knowsAbout." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: `${knowsAboutList.length} topical vectors linked for AI retrieval.` };
  })();

  const searchStatus: FieldStatus = (() => {
    if (!searchActionNode) {
      return { isValid: false, status: "warning", label: "SearchAction Inactive", reason: "SearchAction node missing from @graph." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: "SearchAction active for Google Sitelinks SearchBox." };
  })();

  const faqStatus: FieldStatus = (() => {
    if (!faqNode) {
      return { isValid: false, status: "warning", label: "FAQ Inactive", reason: "FAQPage node missing from @graph." };
    }
    if (!faqQuestions || faqQuestions.length < 2) {
      return { isValid: false, status: "warning", label: "Attention Needed", reason: "Fewer than 2 Q&A pairs in FAQPage schema." };
    }
    return { isValid: true, status: "optimized", label: "Optimized", reason: `${faqQuestions.length} Q&A items structured for Google & Perplexity direct answers.` };
  })();

  const allStatuses = [urlStatus, nameStatus, descStatus, areaStatus, topicStatus, searchStatus, faqStatus];
  ...
  return (
    <div id="entity-card-preview" className="bg-[#0b1329] border border-white/10 rounded-2xl p-4 sm:p-5 text-left space-y-3.5 shadow-2xl relative overflow-hidden flex flex-col h-full font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-bold text-white tracking-tight font-mono">Google Knowledge Graph Card</span>
      ...
      <div className="bg-slate-900/90 border border-white/10 rounded-xl p-2.5 flex flex-wrap items-center justify-between gap-2 font-mono text-xs">
        <div className="flex items-center gap-2">
          <div className={`w-2.5 h-2.5 rounded-full ${overallHealth >= 80 ? "bg-emerald-400 animate-pulse" : "bg-amber-400 animate-pulse"}`} />
          <span className="text-white font-bold">Schema Validation Health: {overallHealth}%</span>
          <span className="text-[10px] text-slate-400">({optimizedCount}/{totalCount} Validated)</span>
        </div>
      ...
      {parseError ? (
        <div className="bg-rose-950/60 border border-rose-500/40 text-rose-200 rounded-xl p-4 text-xs font-mono space-y-2">
          ...
          <p>{parseError}</p>
          <p className="text-[10px] text-slate-400">Fix the JSON syntax in the editor panel to see the live Entity Card preview.</p>
        </div>
      ) : activeTab === "knowledge_panel" ? (
        <div className="space-y-4 flex-1 overflow-y-auto pr-1">
          ...
          <div className="bg-slate-900/90 border border-white/15 rounded-xl p-4 space-y-3.5 shadow-xl relative">
            ...
            <div className={`p-2.5 rounded-xl border transition-all ${
              urlStatus.isValid ? "bg-black/30 border-white/10" : "bg-amber-950/20 border-amber-500/40"
            }`}>
              <div className="flex items-center justify-between relative group/prop">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col text-left leading-none">
                    <span className="text-[11px] font-medium text-slate-200 font-mono">{cleanDomain}</span>
                    <span className="text-[9px] text-slate-400 font-mono truncate max-w-[180px]">{displayUrl}</span>
                  </div>
                </div>
            ...
            <div className={`p-3 rounded-xl border transition-all relative group/prop space-y-2 ${
              ...
              <div className="flex items-start justify-between gap-2 border-b border-white/10 pb-2">
                <div>
                  <h3 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">
                    ...
                    <span className="w-4 h-4 rounded-full bg-sky-500 text-slate-950 inline-flex items-center justify-center text-[10px] font-bold shrink-0" title="Google Knowledge Panel Verified">
                      ✓
                    </span>
                  </h3>
                  <p className="text-xs text-emerald-400 font-mono mt-0.5 font-medium flex items-center gap-1">
                    <Building2 size={12} />
                    <span>Home Energy Upgrades & BER Optimization Organization</span>
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              ...
              <div className={`border transition-colors rounded-xl p-3 space-y-1.5 relative group/prop ${
                areaStatus.isValid ? "bg-black/40 border-white/10 hover:border-emerald-500/40" : "bg-amber-950/20 border-amber-500/40"
              }`}>
                <PropertyTooltip 
                  propName="areaServed" 
                ...
                <div className="flex flex-wrap gap-1 pt-0.5">
                  {selectedAreas && selectedAreas.length > 0 ? (
                    selectedAreas.slice(0, 5).map((area) => (
                      <span key={area} className="bg-emerald-950/80 border border-emerald-500/30 text-emerald-200 text-[10px] font-mono px-1.5 py-0.2 rounded">
                        {area}
                      </span>
                    ))
                  ) : (
                    <span className="text-[10px] font-mono text-amber-300/80 italic">Broad coverage (No specific areas selected)</span>
                  )}
              ...
              <div className={`border transition-colors rounded-xl p-3 space-y-1.5 relative group/prop ${
                topicStatus.isValid ? "bg-black/40 border-white/10 hover:border-sky-500/40" : "bg-amber-950/20 border-amber-500/40"
              }`}>
                <PropertyTooltip 
                  propName="knowsAbout" 
                  schemaType="Text / URL" 
                ...
                <div className="flex flex-wrap gap-1 pt-0.5">
                  {knowsAboutList.map((topic, i) => (
                    <span key={i} className="bg-sky-950/80 border border-sky-500/30 text-sky-200 text-[10px] font-mono px-1.5 py-0.2 rounded">
                      {topic}
                    </span>
                  ))}
                </div>
            ...
            <div className={`border transition-colors rounded-xl p-3 space-y-2 relative group/prop ${
              searchStatus.isValid ? "bg-black/50 border-white/10 hover:border-indigo-500/40" : "bg-amber-950/20 border-amber-500/40"
            }`}>
              <PropertyTooltip 
            ...
            <div className={`space-y-2 p-3 rounded-xl border transition-all relative group/prop ${
              faqStatus.isValid ? "bg-black/40 border-white/10" : "bg-amber-950/20 border-amber-500/40"
            }`}>
              <PropertyTooltip 
                propName="FAQPage" 
                schemaType="FAQPage" 
              ...
              {faqQuestions && faqQuestions.length > 0 ? (
                <div className="space-y-1.5 pt-1">
                  {faqQuestions.map((faq: any, idx: number) => {
                    ...
                    const qText = faq.name || faq.question || `Question #${idx + 1}`;
                    const aText = faq.acceptedAnswer?.text || faq.answer || "Detailed structured answer provided in JSON-LD script.";

                    return (
                      <div key={idx} className="bg-black/40 border border-white/10 hover:border-emerald-500/30 rounded-lg overflow-hidden transition relative group/prop">
                        <PropertyTooltip 
                        ...
                        >
                          ...
                          {isOpen ? <ChevronUp size={13} className="text-emerald-400 shrink-0" /> : <ChevronDown size={13} className="text-slate-400 shrink-0" />}
                        </button>
                        {isOpen && (
        ...
        <div className="space-y-3.5 flex-1 overflow-y-auto pr-1">
          <div className="bg-slate-900/90 border border-emerald-500/30 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 border-b border-white/10 pb-2.5">
              ...
              <div>
                <h4 className="text-xs font-bold text-white font-mono">Conversational AI Engine Indexing View</h4>
                <p className="text-[10px] text-slate-400 font-mono">How Perplexity, ChatGPT & Gemini synthesize your entity data</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5 text-xs text-slate-300">
              <div className="bg-black/50 border border-white/10 hover:border-emerald-500/40 rounded-lg p-3 space-y-1 relative group/prop transition-colors">
                ...
                />
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-[#34d399] font-bold block">1. Identified Authority Entity:</span>
                  <FieldStatusBadge status={nameStatus} />
                </div>
                <p className="font-mono text-slate-200 font-semibold">{displayTitle} ({cleanDomain})</p>
              </div>
              ...
              <div className="bg-black/50 border border-white/10 hover:border-sky-500/40 rounded-lg p-3 space-y-1 relative group/prop transition-colors">
                ...
                />
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-sky-400 font-bold block">2. Core Topical Competencies:</span>
                  <FieldStatusBadge status={topicStatus} />
                </div>
                <p className="text-[11px] text-slate-300">{knowsAboutList.join(" • ")}</p>
              </div>
              ...
              <div className="bg-black/50 border border-white/10 hover:border-amber-500/40 rounded-lg p-3 space-y-1 relative group/prop transition-colors">
                ...
                />
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-amber-300 font-bold block">3. Local Citation Anchors:</span>
                  <FieldStatusBadge status={areaStatus} />
                </div>
              ...
              <div className="bg-black/50 border border-white/10 hover:border-indigo-500/40 rounded-lg p-3 space-y-1 relative group/prop transition-colors">
                ...
                />
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-indigo-300 font-bold block">4. LLM Search Citation Readiness:</span>
                  <span className="text-[10px] font-mono text-emerald-400 font-bold">{overallHealth}% Confidence</span>
                </div>
                <p className="text-[11px] text-slate-300">
                  {overallHealth >= 80 
        ...
        <div className="space-y-3 flex-1 overflow-y-auto pr-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-slate-400">@graph hierarchy</span>
          </div>

          <div className="space-y-2">
            {graphNodes.map((node, i) => {
              const nodeType = node["@type"] || "Thing";
              const isNodeValid = Boolean(node["@type"] && (node["@id"] || node.name || node.url));

              return (
                <div key={i} className="bg-black/50 border border-white/10 hover:border-emerald-500/40 rounded-xl p-3 text-xs font-mono space-y-1.5 relative group/prop transition-colors">
                  <PropertyTooltip 
                    propName={nodeType} 
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-slate-400">{node["@id"] || `node_${i}`}</span>
                  </div>
                  {node.name && <p className="text-white font-semibold text-xs">{node.name}</p>}
