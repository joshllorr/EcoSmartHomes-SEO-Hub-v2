...
export const UnifiedAnalytics: React.FC = () => {
  ...
  if (!data) {
    return <div style={{ color: "#9aa0c2", padding: "40px", textAlign: "center" }}>Loading Unified Analytics...</div>;
  }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Unified Analytics & Predictive Intelligence</h2>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.metricRow}>
            ...
            <span style={styles.metricValue}>{renderTrendTag(trends?.contentVelocityTrend)}</span>
          </div>
          <div style={styles.metricRow}>
          ...
          <div style={styles.metricRow}>
            ...
            <span style={styles.metricValue}>{hub.competitorDiffs} patches</span>
          </div>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.metricRow}>
            ...
            <span style={{ ...styles.metricValue, color: harbor.serpVolatility === "Low" ? "#10b981" : "#f59e0b" }}>
              {harbor.serpVolatility} {renderTrendTag(trends?.serpVolatilityTrend)}
            </span>
          </div>
          <div style={styles.metricRow}>
            <span style={styles.metricLabel}>Backlinks Built</span>
            <span style={styles.metricValue}>
              {harbor.backlinksBuilt} {renderTrendTag(trends?.backlinkGrowthTrend)}
            </span>
          </div>
          <div style={styles.metricRow}>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.metricRow}>
            ...
            <span style={{ ...styles.metricValue, color: "#10b981" }}>{harbor.cmsStatus}</span>
          </div>
          <div style={styles.metricRow}>
            <span style={styles.metricLabel}>Pillar Readiness</span>
            <span style={styles.metricValue}>
              {harbor.pillarReadiness} {renderTrendTag(trends?.pillarStrengthTrend)}
            </span>
          </div>
          <div style={styles.metricRow}>
            <span style={styles.metricLabel}>Pre-Flight Guard</span>
            <span style={{ ...styles.metricValue, color: "#10b981" }}>Active ✈️</span>
          </div>
          <div style={styles.metricRow}>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.metricRow}>
            ...
            <span style={{ ...styles.metricValue, color: "#10b981" }}>{fleet.totalFleetTraffic}</span>
          </div>
          <div style={styles.metricRow}>
      ...
      {autonomousState && (
        <div style={{ ...styles.card, marginTop: "10px", borderColor: "#818cf8" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <div>
              <h3 style={{ ...styles.cardHeader, borderBottom: "none", margin: 0, color: "#a5b4fc", fontSize: "18px" }}>
                🔮 Predictive Engine with Pre-Flight Checklist Safety Guard
              </h3>
              <p style={{ color: "#94a3b8", fontSize: "13px", margin: "4px 0 0 0" }}>
                Actions evaluate Hub online status, queue bounds, task conflicts, manual overrides & 3-minute cooldowns before execution.
              </p>
            </div>
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <span style={{
                background: autonomousState.autoPilotEnabled ? "rgba(16, 185, 129, 0.15)" : "rgba(245, 158, 11, 0.15)",
                color: autonomousState.autoPilotEnabled ? "#10b981" : "#f59e0b",
                padding: "6px 14px",
                borderRadius: "20px",
                fontSize: "12px",
                fontWeight: 700
              }}>
                {autonomousState.autoPilotEnabled ? "🟢 PREDICTIVE AUTO-PILOT" : "⏸️ AUTO-PILOT PAUSED"}
              </span>
              <button
                ...
                style={{
                  ...styles.actionBtn,
                  background: autonomousState.autoPilotEnabled ? "#374151" : "#10b981",
                  color: "#ffffff"
                }}
              >
                {autonomousState.autoPilotEnabled ? "Pause Auto-Pilot" : "Enable Auto-Pilot"}
              </button>
              <button
              ...
              >
                {isProcessing ? "Forecast & Executing..." : "Run Predictive Engine 🔮"}
              </button>
            </div>
          </div>

          <h4 style={{ color: "#cbd5e1", fontSize: "14px", margin: "16px 0 12px 0" }}>Predictive Pre-Flight Optimization Ledger</h4>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {autonomousState.decisionsHistory.slice(0, 6).map((dec) => {
              ...
              return (
                <div key={dec.id} style={{
                  ...
                  alignItems: "center",
                  background: dec.isPredictive ? "rgba(99, 102, 241, 0.08)" : "rgba(15, 23, 42, 0.6)",
                  border: isDeferred ? "1px solid rgba(245, 158, 11, 0.4)" : dec.isPredictive ? "1px solid rgba(99, 102, 241, 0.3)" : "1px solid rgba(255, 255, 255, 0.08)",
                  padding: "12px 16px",
                  borderRadius: "8px"
                }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <span style={{
                        background: meetsThreshold ? "rgba(16, 185, 129, 0.2)" : "rgba(245, 158, 11, 0.2)",
                      ...
                      <strong style={{ color: "#38bdf8", fontSize: "13px" }}>{dec.siteId}</strong>
                      <span style={{ color: "#94a3b8", fontSize: "13px" }}>/ {dec.slug}.html</span>
                      <span style={{
                        background: "#1e293b",
                        color: "#e2e8f0",
                        padding: "2px 8px",
                    ...
                    <span style={{ color: "#cbd5e1", fontSize: "13px" }}>💡 {dec.reason}</span>
                    {isDeferred && (
                  ...
                  <div>
                    {dec.executed ? (
                      ...
                      <button
                        onClick={() => sendHubCommand(dec.action as any, { siteId: dec.siteId, slug: dec.slug })}
                        style={{ ...styles.actionBtn, fontSize: "12px", padding: "4px 10px" }}
                      >
                        {!meetsThreshold ? "Review & Execute" : "Execute Manual"}
                      </button>
                    )}
                  </div>
                </div>
            ...
            {autonomousState.decisionsHistory.length === 0 && (
              <span style={{ color: "#94a3b8", fontSize: "13px", fontStyle: "italic" }}>
                No predictive decisions evaluated yet. Click "Run Predictive Engine" to trigger evaluation.
              </span>
            )}
          </div>
        </div>
      ...
      {data.insights && data.insights.length > 0 && (
        <div style={{ ...styles.card, marginTop: "10px", borderColor: "#3dd68c" }}>
          <h3 style={{ ...styles.cardHeader, color: "#3dd68c" }}>🧠 Autonomous Insights Engine</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {data.insights.map((insight, idx) => (
              <div key={idx} style={styles.insightRow}>
                <span style={styles.insightText}>💡 {insight.text}</span>
                <button 
                  onClick={() => sendHubCommand(insight.action as any, { slug: "auto-generated" })} 
        ...
        <div style={{ display: "flex", gap: "12px", overflowX: "auto", paddingBottom: "8px" }}>
          {events.slice(0, 5).map(e => (
            <div key={e.id} style={styles.eventPill}>
              <strong style={{ color: "#f1f5f9" }}>{e.type}</strong> — {e.message}
            </div>
          ))}
          {events.length === 0 && <span style={styles.metricLabel}>Waiting for live events...</span>}
        </div>
      </div>
