...
export default function ContentAuditTab({
    ...
    return (
      <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
        ...
        <h3 className="font-display font-semibold text-xl text-white">
          No drafts available to audit
        </h3>
        <p className="text-slate-400 text-xs max-w-sm mt-2 leading-relaxed">
          Create an article draft using the AI Writer tool first, then return
          here to run deep SEO diagnostic sweeps, check readability, and
  ...
  // -------------------------------------------------------------
  const wordCount =
    activeDraft.content.split(/\s+/).filter(Boolean).length || 1;
  const sentenceCount =
    activeDraft.content.split(/[.!?]+/).filter((s) => s.trim().length > 0)
      .length || 1;
  ...
  if (avgSentenceLength > 18) {
    ...
    readabilityScore = Math.max(
      30,
      Math.round(100 - (avgSentenceLength - 15) * 4.5),
    );
  ...
  const targetKeywords =
    activeDraft.keywords && activeDraft.keywords.length > 0
  ...
  const overallSEOAuditScore = Math.round(
    (readabilityScore + densityScore + metaScore) / 3,
  );
  ...
  const handleFix = async (metricType: 'meta' | 'density' | 'readability') => {
    ...
    try {
      ...
      const updated: ArticleDraft = { ...activeDraft };
      ...
      if (data.warning) {
        ...
        setSuccessMsg(
          `Successfully optimized your article's ${metricType === 'meta' ? 'Google meta tags' : metricType === 'density' ? 'keyword density' : 'readability'}!`,
        );
      }
      ...
      console.error(err);
      setErrorMsg(
        'Failed to apply automatic SEO optimization. Please check your connection.',
      );
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">
            Run complete linguistic sweeps on drafts for readability, keyword
            depth, and search snippet optimization.
          </p>
        </div>
        ...
        <div className="glass-card p-6 flex flex-col justify-between relative overflow-hidden col-span-1">
          ...
          <div>
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400">
              Combined SEO Strength
            </h3>
            <p className="text-[10px] text-slate-400 mt-1 leading-normal">
              An aggregate audit computed from target density weightings,
              average sentence structures, and metadata bounds.
            </p>
          </div>
            ...
            <div className="relative w-32 h-32 flex items-center justify-center">
              ...
              >
                ...
                <circle
                  ...
                  r="40"
                  stroke={
                    overallSEOAuditScore > 80
                      ? '#34d399'
                      : overallSEOAuditScore > 50
                        ? '#f59e0b'
                        : '#ef4444'
                  }
                  strokeWidth="8"
                  ...
                  strokeDashoffset={
                    251.2 - (251.2 * overallSEOAuditScore) / 100
                  }
          ...
          <div className="flex items-center gap-2 bg-white/5 px-3 py-2 rounded-lg border border-white/5 text-[10px] text-slate-400 leading-tight">
            ...
            <span>
              Targeting 85%+ secures optimal search indexing on modern search
              networks.
            </span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 col-span-1 lg:col-span-2 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Visualize how your draft will appear to Irish homeowners searching
              for retrofitting assistance.
            </p>
          </div>
          ...
          <div className="bg-[#1e1e1e] border border-white/5 p-4 rounded-xl space-y-2 font-sans my-4 shadow-inner">
            <div className="flex items-center gap-1.5 text-xs text-[#dadce0]">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] text-[#9aa0a6] leading-none">
                  https://{targetDomain} › blog › retrofit
                </span>
              </div>
            </div>
            ...
            <p className="text-xs text-[#bdc1c6] leading-relaxed line-clamp-2">
              {mDesc ||
                "No meta description generated yet. Google will crawl your page and display an auto-generated summary which can hurt search click-through rates. Click 'Fix Meta' below to resolve."}
            </p>
          </div>
      ...
      <AnimatePresence mode="wait">
        {successMsg && (
          ...
          >
            ...
            <span>
              {successMsg} 🚀 <strong>+15 XP Earned!</strong>
            </span>
          </motion.div>
        )}
        {warningMsg && (
          ...
          >
            ...
            <span>
              {warningMsg} <strong>+15 XP Earned!</strong>
            </span>
          </motion.div>
        )}
        {errorMsg && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-black/15 border border-white/5 rounded-xl overflow-hidden text-xs my-2">
              ...
              <div className="divide-y divide-white/5">
                {keywordMatches.map((m, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-3 px-3 py-2 text-slate-300"
                  >
                    <span className="truncate font-medium">{m.keyword}</span>
                    <span className="text-center font-mono">{m.count}</span>
                    <span
                      className={`text-right font-mono ${m.density > 0 ? 'text-slate-200' : 'text-slate-500'}`}
                    >
                      {m.density}%
                    </span>
                  </div>
        ...
        <div className="glass-card p-6 flex flex-col justify-between space-y-4">
          <div className="space-y-2">
            ...
            <div className="bg-black/15 border border-white/5 rounded-xl p-3.5 space-y-2.5 text-xs text-slate-300">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono font-bold text-white">
                  {avgSentenceLength} words/sentence
                </span>
              </div>
              <div className="flex justify-between">
          ...
          <button
            ...
            disabled={
              fixingMetric !== null || readabilityStatus === 'Optimized'
            }
            className={`w-full py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer ${
              readabilityStatus === 'Optimized'
                ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 cursor-default'
        ...
        <div className="glass-card p-6 flex flex-col justify-between space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400 leading-normal">
              Structured titles and descriptions prevent truncation, raise
              organic Click-Through-Rates (CTR), and inform search spiders.
            </p>
            ...
            <div className="bg-black/15 border border-white/5 rounded-xl p-3.5 space-y-3.5 text-xs">
              <div className="space-y-1">
                ...
                <div className="bg-white/5 p-2 rounded border border-white/5 text-slate-300 font-mono text-[10px] truncate">
                  {mTitle || (
                    <em className="text-slate-500">Missing Title tag!</em>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="bg-white/5 p-2 rounded border border-white/5 text-slate-300 font-mono text-[10px] line-clamp-2 leading-relaxed">
                  {mDesc || (
                    <em className="text-slate-500">Missing Description tag!</em>
                  )}
                </div>
              </div>
            </div>
