...
export default function SERPViewer({ serp, onSendToWriter }: SERPViewerProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "competitors" | "gaps" | "outline">("overview");
  const [copiedOutline, setCopiedOutline] = useState(false);

  if (!serp) {
    return (
      <div className="bg-[#0f172a]/40 border-2 border-dashed border-white/10 rounded-2xl p-8 text-center flex flex-col items-center justify-center min-h-[220px]" id="serp-empty-state">
        ...
        <h4 className="font-display font-semibold text-slate-200 text-sm">No SERP analysis yet</h4>
  ...
  const getDifficultyColor = (score: number) => {
    if (score < 30) return "bg-emerald-500/20 text-[#34d399] border-emerald-500/30";
  ...
  const getIntentColor = (intent: string) => {
    ...
    if (term.includes("transactional")) return "bg-purple-500/20 text-purple-300 border-purple-500/30";
    if (term.includes("commercial")) return "bg-blue-500/20 text-blue-300 border-blue-500/30";
    if (term.includes("local")) return "bg-teal-500/20 text-teal-300 border-teal-500/30";
    return "bg-amber-500/20 text-amber-300 border-amber-500/30";
  };

  const handleCopyOutline = () => {
    const text = serp.recommended_outline.map((line, i) => `${i + 1}. ${line}`).join("\n");
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-300" id="serp-viewer-container">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-white/10 pb-4">
        <div>
          ...
          <h2 className="text-xl md:text-2xl font-display font-bold text-white tracking-tight flex items-center gap-2 mt-1">
            ...
            <span>Organic Target: <em className="text-[#34d399] not-italic font-mono font-semibold">“{serp.keyword}”</em></span>
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            Expanded competitor landscape, ranking gaps, and strategic content opportunities
          </p>
        </div>

        {/* Quick status pills */}
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${getIntentColor(serp.intent)}`}>
            <strong>Intent:</strong> {serp.intent}
          </span>
          <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${getDifficultyColor(serp.difficulty)}`}>
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          ...
          {serp.summary_markdown && (
            <div className="glass-card p-6 space-y-4 border-l-4 border-l-[#34d399]" id="serp-summary-card">
            ...
            <div className="glass-card p-5 space-y-3" id="serp-opportunities-card">
              ...
              <ul className="space-y-2.5 text-xs text-slate-300">
                {serp.opportunities.map((o, i) => (
                  <li key={i} className="flex items-start gap-2.5 leading-relaxed bg-white/5 p-3 rounded-xl border border-white/5 hover:border-white/10 transition">
            ...
            <div className="glass-card p-5 space-y-3" id="serp-outline-preview-card">
              ...
              <ul className="space-y-2 text-xs text-slate-300">
                {serp.recommended_outline.map((o, i) => (
                  <li key={i} className="flex items-start gap-2 leading-relaxed bg-white/5 p-2.5 rounded-lg border border-white/5">
                    <span className="text-sky-400 font-bold shrink-0 font-mono text-xs">{i + 1}.</span>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {serp.top_results.map((r) => (
              <div key={r.position} className="glass-card p-5 hover:border-white/15 transition duration-200" id={`serp-result-pos-${r.position}`}>
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-sky-400 font-mono mt-1 hover:underline truncate max-w-xl flex items-center gap-1">
                      <a href={r.url} target="_blank" rel="noreferrer" className="flex items-center gap-1">
                        <span>{r.url}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 pt-4 border-t border-white/5">
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-slate-400 block">Themes:</span>
                  ...
                  {/* Strengths */}
                  <div className="space-y-1.5">
                    <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-[#34d399] block">Strengths:</span>
                    <ul className="space-y-1 text-slate-300 text-xs">
                      {r.strengths.map((t, i) => (
                        <li key={i} className="flex items-start gap-1.5 leading-normal">
                          <span className="text-[#34d399] font-bold shrink-0">✓</span>
                  ...
                  {/* Weaknesses */}
                  <div className="space-y-1.5">
                    <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-rose-400 block">Weaknesses:</span>
                    <ul className="space-y-1 text-slate-300 text-xs">
                  ...
                  {/* Ranking Gaps */}
                  <div className="space-y-1.5">
                    <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-sky-400 block">Content Gaps:</span>
                    <ul className="space-y-1 text-slate-300 text-xs">
              ...
              <tbody className="divide-y divide-white/5">
                {(serp.ranking_gap_keywords || [
                  ...
                  <tr key={idx} className="hover:bg-white/5 transition">
                    ...
                    <td className="p-3 text-center font-mono">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getDifficultyColor(gap.difficulty)}`}>
                        {gap.difficulty}
                      </span>
                    </td>
                    <td className="p-3 text-center font-mono">
      ...
      {activeTab === "outline" && (
        <div className="glass-card p-6 space-y-5 border border-sky-500/20">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                <span>{copiedOutline ? "Copied to Clipboard" : "Copy Outline"}</span>
              </button>
          ...
          <div className="space-y-3">
            {serp.recommended_outline.map((o, i) => (
              <div key={i} className="flex items-start gap-3 bg-white/5 p-3.5 rounded-xl border border-white/5 hover:border-white/15 transition">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1 leading-normal">
                    Target focus keywords: <span className="text-slate-300 font-mono">{serp.keyword}</span>, <span className="text-slate-300 font-mono">BER upgrade Ireland</span>, <span className="text-slate-300 font-mono">SEAI grant steps</span>.
                  </p>
                </div>
              </div>
            ))}
