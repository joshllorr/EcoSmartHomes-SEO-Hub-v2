...
import { Flame, Sparkles, TrendingUp, HelpCircle, Calendar, Info, Award } from "lucide-react";
...
export default function SEOHeatmapCard({ data }: SEOHeatmapCardProps) {
  ...
  const defaultHeatmap: HeatmapDay[] = [
    { day: "Mon", visibility: 42, discovery_sessions: 1, ctr: 1.8, rankings: 12 },
    { day: "Tue", visibility: 45, discovery_sessions: 2, ctr: 2.1, rankings: 14 },
    { day: "Wed", visibility: 68, discovery_sessions: 4, ctr: 3.5, rankings: 25 },
    { day: "Thu", visibility: 52, discovery_sessions: 1, ctr: 2.4, rankings: 18 },
    { day: "Fri", visibility: 75, discovery_sessions: 3, ctr: 3.8, rankings: 29 },
    { day: "Sat", visibility: 48, discovery_sessions: 0, ctr: 2.0, rankings: 16 },
    { day: "Sun", visibility: 40, discovery_sessions: 1, ctr: 1.6, rankings: 11 }
  ];
  ...
  const [activeMetric, setActiveMetric] = useState<"visibility" | "ctr" | "rankings">("visibility");
        ...
        <div className="bg-slate-900/95 border border-white/15 p-3 rounded-xl shadow-xl text-left font-sans text-xs">
          ...
          <div className="space-y-1 text-slate-300">
            <p className="flex justify-between gap-4">
              ...
              <span className="font-mono text-[#34d399] font-bold">{payload[0].payload.visibility}%</span>
            </p>
            <p className="flex justify-between gap-4">
              <span>Click-Through Rate:</span>
              <span className="font-mono text-sky-400 font-bold">{payload[0].payload.ctr}%</span>
            </p>
            <p className="flex justify-between gap-4">
              <span>Top 10 Keywords:</span>
              <span className="font-mono text-purple-400 font-bold">{payload[0].payload.rankings}</span>
            </p>
            <p className="flex justify-between gap-4 border-t border-white/5 pt-1 mt-1 text-[10px]">
              <span>Scans Run:</span>
              <span className="font-mono text-amber-400 font-bold">{payload[0].payload.discovery_sessions}</span>
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-6 text-left" id="seo-heatmap-card">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h3 className="text-sm font-semibold text-white mt-1.5">7-Day SEO Performance Heatmap</h3>
        </div>
      ...
      <div className="grid grid-cols-7 gap-2.5">
        {heatmapData.map((dayData, idx) => {
          ...
          const score = 
            ...
            activeMetric === "ctr" ? Math.round(dayData.ctr * 20) :
          ...
          return (
            <button
              ...
              className={`p-3 rounded-xl border text-center transition flex flex-col items-center justify-between gap-1.5 group cursor-pointer ${getHeatIntensityStyle(score)} ${
                isSelected ? "ring-2 ring-[#34d399] scale-[1.03]" : "hover:scale-[1.01]"
              }`}
            >
              <span className="text-[10px] font-mono font-bold tracking-wider opacity-70 uppercase">{dayData.day}</span>
              <div className="flex flex-col items-center justify-center">
                ...
                {isHighPerformer(dayData.day) && (
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-ping mt-1" title="High Performing Discovery Day" />
                )}
              </div>
            </button>
          );
        ...
        <div className="lg:col-span-8 bg-black/20 border border-white/10 rounded-2xl p-4 flex flex-col h-[230px] relative">
          <div className="flex justify-between items-center mb-2.5 px-1.5">
            <span className="text-[10px] font-mono text-slate-400 uppercase font-semibold flex items-center gap-1">
              ...
              {activeMetric === "visibility" && "Weekly Visibility Curve & Scans"}
          ...
          <div className="flex-1 min-h-[170px] select-none text-xs">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <ReferenceLine x="Wed" stroke="#fbbf24" strokeDasharray="3 3" opacity={0.3} />
                <ReferenceLine x="Fri" stroke="#fbbf24" strokeDasharray="3 3" opacity={0.3} />
        ...
        <div className="lg:col-span-4 bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col justify-between text-left relative overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-slate-200">Insights for {selectedDay.day}</span>
              </div>
              {isHighPerformer(selectedDay.day) ? (
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="bg-black/20 rounded-xl p-2.5 border border-white/5">
                <span className="text-[9px] font-mono text-slate-400 block leading-tight">VISIBILITY INDEX</span>
                <span className="text-base font-bold font-mono text-[#34d399] mt-1 block">
                  {selectedDay.visibility}%
                </span>
              </div>
              <div className="bg-black/20 rounded-xl p-2.5 border border-white/5">
                <span className="text-[9px] font-mono text-slate-400 block leading-tight">ORGANIC CTR</span>
                <span className="text-base font-bold font-mono text-sky-400 mt-1 block">
                  {selectedDay.ctr}%
                </span>
              </div>
              <div className="bg-black/20 rounded-xl p-2.5 border border-white/5">
                <span className="text-[9px] font-mono text-slate-400 block leading-tight">TOP-10 RANKINGS</span>
                <span className="text-base font-bold font-mono text-purple-300 mt-1 block">
                  {selectedDay.rankings}
                </span>
              </div>
              <div className="bg-black/20 rounded-xl p-2.5 border border-white/5">
                <span className="text-[9px] font-mono text-slate-400 block leading-tight">SCANS RUN</span>
          ...
          <div className="mt-4 pt-3.5 border-t border-white/5 flex gap-2.5">
            <div className="p-1.5 bg-[#34d399]/10 rounded-lg h-fit border border-[#34d399]/20 shrink-0">
              <Sparkles size={13} className="text-[#34d399] fill-[#34d399]/10" />
            </div>
            <div>
              <span className="text-[9px] font-bold text-[#34d399] uppercase font-mono tracking-wider block">AI SEO ADVISOR</span>
      ...
      <div className="flex gap-2 p-3 rounded-xl bg-white/5 border border-white/10 text-slate-400 text-[10px] items-start">
        ...
        <p className="leading-normal">
          <strong>How to read:</strong> The heatmap tracks daily organic SEO visibility based on Google search console impressions. High intensity days (Wednesday & Friday) highlight the highest search frequencies for Dublin energy upgrades, proving that content research and indexing yield peak organic click-throughs on these specific days.
        </p>
      </div>
    </div>
  );
