...
import { MessageSquare, Bot, User, RefreshCw, Sparkles, ShieldCheck, Euro, Compass, TrendingUp, HelpCircle } from "lucide-react";
      ...
      <div className="glass-card p-6 border border-emerald-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 37 AI Copilot Monitoring</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">Homeowner AI Advisor Session Telemetry</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">Real-time intelligence on homeowner queries, upgrade interest, and contractor concerns.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-xs">
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-sky-400">
            ...
            <span className="font-bold text-slate-300">Active Copilot Sessions</span>
          </div>
          <span className="text-3xl font-bold text-sky-300 mt-3">{metrics.activeSessions}</span>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          ...
          <span className="text-3xl font-bold text-emerald-400 mt-3">{metrics.totalMessagesExchanged}</span>
        </div>

        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-indigo-400">
            <Compass size={18} />
            <span className="font-bold text-slate-300">Primary Inquiry Category</span>
          </div>
          <span className="text-sm font-bold text-indigo-300 mt-3">{metrics.topInquiryCategory}</span>
        </div>

        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-emerald-400">
            <ShieldCheck size={18} />
            <span className="font-bold text-slate-300">Satisfaction Rating</span>
          </div>
          <span className="text-3xl font-bold text-emerald-300 mt-3">{metrics.satisfactionRate}</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {metrics.commonQuestions.map((q, idx) => (
            <div key={idx} className="p-4 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col justify-between gap-2">
              <span className="text-[10px] text-slate-400 uppercase font-bold">{q.topic}</span>
              <div className="flex justify-between items-baseline mt-1">
                <span className="text-xl font-bold text-emerald-400">{q.count}</span>
