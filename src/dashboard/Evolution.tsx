...
import { Dna, Award, RefreshCw, TrendingUp, Cpu, CheckCircle2 } from "lucide-react";
    ...
    strategy: {
      ...
      planHistory: [
        ...
        },
        {
          ...
          plan: [{ type: "adjust-regions", reason: "Expand high-performing Irish counties" }],
          longReward: 0.84,
          simulatedReward: 0.80
        }
      ]
    },
    simulation: {
  ...
  return (
    <div className="flex flex-col gap-5 text-left">
      <div className="glass-card p-6 border border-pink-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-pink-400 font-bold tracking-wider">Phase 20 Multi-Cycle Strategy Evolution</span>
          <h2 className="text-lg font-bold text-white mt-0.5">Strategy Evolution Console</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">Genetic Mutations</span>
          </div>
          <div className="flex items-baseline gap-2 mt-3">
            <span className="text-3xl font-bold font-mono text-pink-400">{mutations}</span>
            <span className="text-xs text-slate-400 font-mono">Generational Cycles</span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">Long-Horizon Reward Trend</span>
          </div>
          <div className="flex items-baseline gap-2 mt-3">
            <span className="text-3xl font-bold font-mono text-emerald-400">{avgReward}</span>
            <span className="text-xs text-slate-400 font-mono">Avg ({rewards.length || 2} cycles)</span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">Market Stress Context</span>
          </div>
          <div className="flex flex-col gap-1 mt-2 text-xs font-mono">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-amber-300 font-bold">{((simulation.simState?.regionalDemandShock ?? 0.15) * 100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {sortedBiases.map(([type, score], i) => (
            <div key={i} className="p-3.5 bg-slate-950/80 border border-white/5 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-2">
