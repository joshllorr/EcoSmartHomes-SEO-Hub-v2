...
import { AlertTriangle, RefreshCw, Users, ShieldCheck, Scale, CheckCircle2, XCircle } from "lucide-react";
...
interface ConflictData {
  ...
  negotiation: {
    approved?: boolean;
    votes?: { agent: string; approve: boolean; confidence: number; notes?: string }[];
  };
  ...
  return (
    <div className="flex flex-col gap-5 text-left">
      ...
      <div className="glass-card p-6 border border-amber-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-amber-400 font-bold tracking-wider">Phase 22 Autonomous Multi-Agent Conflict Resolution</span>
          <h2 className="text-lg font-bold text-white mt-0.5">Multi-Agent Conflict Resolution Console</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            <AlertTriangle size={18} className={conflict?.conflict ? "text-amber-400" : "text-emerald-400"} />
            <span className="text-xs font-mono font-bold text-slate-300">Conflict Status</span>
          </div>
          <div className="flex items-baseline gap-2 mt-3">
            <span className={`text-2xl font-bold font-mono uppercase ${conflict?.conflict ? "text-amber-400" : "text-emerald-400"}`}>
              {conflict?.conflict ? "Conflict Active & Resolved" : "No Active Disagreement"}
            </span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">Council Votes Ratio</span>
          </div>
          <div className="flex items-baseline gap-3 mt-3 font-mono">
            <span className="text-2xl font-bold text-emerald-400">{conflict?.approvals ?? 2} Approved</span>
            <span className="text-2xl font-bold text-rose-400">{conflict?.rejections ?? 1} Rejection</span>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-300">Bias Divergence Spread</span>
          </div>
          <div className="flex items-baseline gap-2 mt-3 font-mono">
            <span className="text-3xl font-bold text-purple-300">{(conflict?.biasSpread ?? 0.23).toFixed(2)}</span>
        ...
        <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            {(negotiation?.votes || []).map((v, idx) => (
              <div key={idx} className="p-3.5 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono font-bold text-white">{v.agent}</span>
                  </div>
                  <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded ${v.approve ? "bg-emerald-500/20 text-emerald-300" : "bg-rose-500/20 text-rose-300"}`}>
                    {v.approve ? "APPROVED" : "REJECTED"}
                  </span>
                </div>
                {v.notes && <p className="text-[11px] text-slate-400 font-mono pl-6">{v.notes}</p>}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {sortedBiases.map(([type, weight], idx) => (
              <div key={idx} className="p-3.5 bg-slate-950/80 border border-white/5 rounded-xl flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-white uppercase">{type}</span>
