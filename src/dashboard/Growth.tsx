...
export default function Growth() {
  ...
  const getBiasBadge = (bias: string) => {
    switch (bias) {
      ...
      case 'defensive':
        return (
          <span className="px-3 py-1 bg-rose-500/20 text-rose-300 font-mono font-bold text-xs rounded-full">
            🛡️ DEFENSIVE REDUCTION
          </span>
        );
      case 'balanced':
      default:
        return (
          <span className="px-3 py-1 bg-sky-500/20 text-sky-300 font-mono font-bold text-xs rounded-full">
      ...
      <div className="glass-card p-6 border border-emerald-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h2 className="text-lg font-bold text-white mt-0.5">
            Predictive Growth Engine Console
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-5 glass-card border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-mono font-bold text-emerald-400 uppercase">
              {growthState.backlinkTrend}
            </span>
            <span className="text-xs font-mono text-slate-300">
              Slope: +{growthState.backlinkSlope}
            </span>
          </div>
          <span className="text-[11px] text-slate-400">
            Historical crawl trajectory
          </span>
        </div>

        <div className="p-5 glass-card border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-2">
          <span className="text-xs font-bold text-slate-400">
            Heatmap Demand Trend
          </span>
          <div className="flex items-center justify-between">
            <span className="text-sm font-mono font-bold text-sky-400 uppercase">
              {growthState.heatmapTrend}
            </span>
            <span className="text-xs font-mono text-slate-300">
              Slope: +{growthState.heatmapSlope}
            </span>
          </div>
          <span className="text-[11px] text-slate-400">
            Irish county search velocity
          </span>
        </div>
        ...
        <div className="p-5 glass-card border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-2">
          ...
          <span className="text-sm font-mono font-bold text-emerald-300">
            {growthState.forecast30Day}
          </span>
          <span className="text-[11px] text-emerald-400 font-semibold">
            100% Growth Alignment
          </span>
        </div>
      </div>
    </div>
