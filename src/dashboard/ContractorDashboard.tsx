      ...
      <div className="glass-card p-6 border border-purple-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-purple-400 font-bold tracking-wider">
            Phase 28 Contractor Coordination Engine
          </span>
          <h2 className="text-xl font-bold text-white mt-0.5">
            SEAI Contractor Pool & Retrofit Job Execution
          </h2>
        </div>
        ...
        <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-4 font-mono text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {contractors.map((c, idx) => (
              <div
                key={idx}
                className="p-4 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col gap-2.5"
              >
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {c.type.map((t: string, i: number) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 bg-purple-500/10 text-purple-300 border border-purple-500/20 rounded text-[10px]"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <div className="flex justify-between items-center text-[11px] text-slate-400 border-t border-white/5 pt-2 mt-1">
                  <span className="flex items-center gap-1">
                    <MapPin size={12} className="text-purple-400" /> Regions:{' '}
                    {c.region.join(', ')}
                  </span>
                  <span className="flex items-center gap-1">
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-purple-300">
                      Target: {j.scheduledDate} @ {j.scheduledTime}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">
                    ...
                    | Homeowner:{' '}
                    <strong className="text-slate-200">{j.homeowner_id}</strong>
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  {j.status === 'scheduled' && (
                    <button
                      onClick={() =>
                        handleUpdateJobStatus(j.job_id, 'in_progress')
                      }
                  ...
                  {j.status === 'in_progress' && (
                    <button
                      onClick={() =>
                        handleUpdateJobStatus(j.job_id, 'completed')
                      }
      ...
      {activeView === 'calendar' && (
        <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-4 font-mono text-xs">
          ...
          <div className="flex flex-col gap-3">
            {[
              {
                measure: 'Air-to-Water Heat Pump Installation',
                contractor: 'GreenHeat Solutions',
                status: 'Sign-off Uploaded',
                step: 'SEAI Grant Submission Ready',
              },
              {
                measure: 'Rooftop Solar PV Array',
                contractor: 'EcoSolar & Electric',
                status: 'Scheduled',
                step: 'Awaiting Installation Date',
              },
              {
                measure: 'Attic Insulation Upgrade',
                contractor: 'Munster Retrofit',
                status: 'Completed',
                step: 'BER Cert Verified',
              },
            ].map((row, idx) => (
              <div
                key={idx}
              ...
              >
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Assigned: {row.contractor} | Compliance: {row.step}
                  </p>
                </div>
