...
import { Smile, Heart, ShieldCheck, RefreshCw, Zap, Euro, Leaf, Clock, Cpu, Award, AlertTriangle, Users, Compass } from "lucide-react";
    ...
    <div className="flex flex-col gap-5 text-left font-sans">
      ...
      <div className="glass-card p-6 border border-emerald-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 38 National Psychological Model</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">Homeowner Sentiment & Emotional Telemetry</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">Macro sentiment intelligence correlated with SEAI timelines, contractor ratings, and AI copilot engagement.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 font-mono text-xs">
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          ...
          <span className="text-3xl font-bold text-emerald-400 mt-3">{metrics.avgConfidence}%</span>
        </div>

        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-sky-400">
            <Compass size={18} />
            <span className="font-bold text-slate-300">Avg Process Clarity</span>
          </div>
          <span className="text-3xl font-bold text-sky-300 mt-3">{metrics.avgClarity}%</span>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          ...
          <span className="text-3xl font-bold text-amber-300 mt-3">{metrics.avgStress}%</span>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          ...
          <span className="text-3xl font-bold text-emerald-300 mt-3">{metrics.avgSatisfaction}%</span>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          ...
          <span className="text-3xl font-bold text-indigo-300 mt-3">{metrics.avgTrust}%</span>
        </div>
      </div>
          ...
          {metrics.correlations.map((c, idx) => (
            <div key={idx} className="p-4 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col justify-between gap-2">
              ...
              <div className="flex justify-between items-baseline mt-1">
                <span className="text-base font-bold text-emerald-400">{c.impact}</span>
