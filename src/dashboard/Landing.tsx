...
import { Layout, Lightbulb, RefreshCw, Eye, ShieldCheck, DollarSign, Activity } from "lucide-react";
...
export default function Landing() {
  ...
  const { timestamp, suggestions, heatmap, fusion, simState, negotiation } = data;
  ...
  return (
    <div className="flex flex-col gap-5 text-left">
      <div className="glass-card p-6 border border-purple-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-purple-400 font-bold tracking-wider">Phase 18 Autonomous CRO Engine</span>
          <h2 className="text-lg font-bold text-white mt-0.5">Autonomous Landing Page Optimizer</h2>
        </div>
      ...
      <div className="glass-card p-6 border border-purple-500/30 rounded-2xl bg-purple-950/10 flex flex-col gap-3">
        <div className="flex justify-between items-center border-b border-purple-500/20 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Actionable CRO Recommendations</h3>
          </div>
        ...
        <div className="flex flex-col gap-2 mt-1">
          {suggestions.map((s, i) => (
            <div key={i} className="p-3 bg-slate-950/80 border border-purple-500/20 rounded-xl flex items-start gap-2.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-white font-mono">Heatmap UX Signals</h4>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              ...
              <span className="text-xs font-mono font-bold text-teal-300">{((heatmap?.scrollDepth ?? 0.42) * 100).toFixed(0)}%</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              <span className="text-xs text-slate-400">CTA Click Concentration</span>
              <span className="text-xs font-mono font-bold text-teal-300">{((heatmap?.clickConcentration ?? 0.28) * 100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-white font-mono">Content Quality Comparison</h4>
          </div>

          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              <span className="text-xs text-slate-400">Your Content Rating</span>
              <span className="text-xs font-mono font-bold text-sky-300">{((fusion?.contentQuality ?? 0.65) * 100).toFixed(0)}%</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              <span className="text-xs text-slate-400">Top Competitor Rating</span>
              <span className="text-xs font-mono font-bold text-indigo-400">{((fusion?.competitorContentQuality ?? 0.82) * 100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-white font-mono">Market Stress & CPC</h4>
          </div>

          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              <span className="text-xs text-slate-400">CPC Volatility</span>
              <span className="text-xs font-mono font-bold text-pink-400">±{((simState?.cpcVolatility ?? 0.28) * 100).toFixed(0)}%</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-slate-950/80 rounded-xl border border-white/5">
              <span className="text-xs text-slate-400">Regional Demand Shock</span>
              <span className="text-xs font-mono font-bold text-amber-400 font-semibold">{((simState?.regionalDemandShock ?? 0.15) * 100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
      </div>
