...
import { ShieldCheck, ShieldAlert, Users, RefreshCw, CheckCircle, XCircle, Zap, Cpu } from "lucide-react";
...
export default function Negotiation() {
  ...
  const { timestamp, approved, reason, votes, longReward, simulatedReward, plan } = data;
  ...
  return (
    <div className="flex flex-col gap-5 text-left">
      <div className="glass-card p-6 border border-teal-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-teal-400 font-bold tracking-wider">Phase 15 Multi-Agent Council</span>
          <h2 className="text-lg font-bold text-white mt-0.5">Multi-Agent Strategy Debate & Consensus Room</h2>
        </div>
      ...
      {/* Consensus Banner */}
      <div className={`glass-card p-6 border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${approved ? "border-emerald-500/30 bg-emerald-950/20" : "border-rose-500/30 bg-rose-950/20"}`}>
        <div className="flex items-center gap-3">
          <div className={`p-3 rounded-xl border ${approved ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-rose-500/10 border-rose-500/30 text-rose-400"}`}>
            {approved ? <ShieldCheck size={24} /> : <ShieldAlert size={24} />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className={`text-xs font-mono font-bold uppercase ${approved ? "text-emerald-400" : "text-rose-400"}`}>
                {approved ? "CONSENSUS OUTCOME: APPROVED" : "CONSENSUS OUTCOME: REJECTED"}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {votes.map((v, idx) => (
          <div key={idx} className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between gap-3">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 font-mono">{v.agent}</span>
                {v.approve ? (
              ...
              <p className="text-xs text-slate-300 mt-2 italic">{v.notes || "Agent review completed."}</p>
            </div>

            <div className="flex justify-between items-center pt-3 border-t border-white/5">
              <span className="text-[10px] text-slate-400">Confidence Rating</span>
              <span className="text-xs font-mono font-bold text-teal-300">{(v.confidence * 100).toFixed(0)}%</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Plan Under Negotiation</h3>
          </div>
