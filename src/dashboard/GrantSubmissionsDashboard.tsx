...
import { FileCheck, Award, TrendingUp, Clock, RefreshCw, CheckCircle2, AlertCircle, ShieldCheck, Eye, Filter, ArrowUpDown, DollarSign } from "lucide-react";
import { apiGet, apiPost } from "../hooks/useApi";
import { StatusBadge, SubmissionTimeline } from "../portal/grants/GrantStatusView";

export default function GrantSubmissionsDashboard() {
  const [navTab, setNavTab] = useState<"table" | "inspector" | "insights" | "timeline">("table");
  ...
  const handleUpdateStatus = async (submissionId: string, newStatus: string, reason?: string) => {
    try {
  ...
  return (
    <div className="flex flex-col gap-5 text-left font-sans">
      ...
      <div className="glass-card p-6 border border-emerald-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 30 SEAI Grant Submission Intelligence Engine</span>
          <h2 className="text-xl font-bold text-white mt-0.5">SEAI Submissions Operations & Analytics</h2>
        </div>
      ...
      <div className="flex items-center gap-2 font-mono text-xs border-b border-white/10 pb-3">
        <button
          ...
          className={`px-4 py-2 rounded-xl font-bold transition cursor-pointer ${
            navTab === "table" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
          }`}
        >
          Submissions Queue
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-xl font-bold transition cursor-pointer ${
            navTab === "inspector" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
          }`}
        >
          Payload Inspector
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-xl font-bold transition cursor-pointer ${
            navTab === "insights" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl font-bold transition cursor-pointer ${
            navTab === "timeline" ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-xs">
            <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
              <div className="flex items-center gap-2 text-sky-400">
                <FileCheck size={18} />
                <span className="font-bold text-slate-300">Total Submissions</span>
              </div>
              <span className="text-3xl font-bold text-sky-300 mt-3">{metrics.totalSubmissions}</span>
            </div>
            ...
            <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
              ...
              <span className="text-3xl font-bold text-emerald-400 mt-3">{metrics.approvalRate}</span>
            </div>

            <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
              <div className="flex items-center gap-2 text-indigo-400">
                <Clock size={18} />
                <span className="font-bold text-slate-300">Avg Approval Time</span>
              </div>
              <span className="text-3xl font-bold text-indigo-300 mt-3">{metrics.averageApprovalDays}</span>
            </div>

            <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
              <div className="flex items-center gap-2 text-purple-400">
                <Award size={18} />
                <span className="font-bold text-slate-300">Disbursed Funds</span>
              </div>
              <span className="text-3xl font-bold text-purple-300 mt-3">{metrics.totalGrantPayout}</span>
            </div>
          </div>
          ...
          <div className="glass-card p-4 border border-white/10 rounded-2xl bg-slate-900/60 flex justify-between items-center font-mono text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300 font-bold">Filter by Status:</span>
              <select
                value={filterStatus}
            ...
            <div className="flex flex-col gap-3">
              {filteredSubmissions.map((sub, idx) => (
                <div key={idx} className="p-4 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-white text-sm">{sub.homeowner?.name || "Homeowner"}</span>
                      <span className="text-sky-300 font-bold">{sub.seaiReference}</span>
                      <StatusBadge status={sub.status} />
                    </div>
                    <p className="text-[11px] text-slate-400 mt-1">
                      Eircode: {sub.homeowner?.eircode} | Total Grant: <strong className="text-emerald-400">€{(sub.totals?.totalGrant || sub.totalGrant || 22100).toLocaleString()}</strong> | Contractor: {sub.contractor?.name || sub.contractor}
                    </p>

                    {sub.statusHistory && sub.statusHistory.length > 0 && (
                      <div className="flex items-center gap-2 mt-2 text-[10px] text-slate-400">
                        <span className="text-slate-500 font-bold">Audit Trail:</span>
                        {sub.statusHistory.map((h: any, hIdx: number) => (
                          <span key={hIdx} className="flex items-center gap-1">
                            <span className="text-sky-400 font-bold">{h.status}</span>
                            {hIdx < sub.statusHistory.length - 1 && <span className="text-slate-600">➔</span>}
                          </span>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {sub.status === "draft" && (
                      <button
                        onClick={() => handleUpdateStatus(sub.submission_id, "ready")}
                    ...
                    {(sub.status === "ready" || sub.status === "draft") && (
                      <button
                        onClick={() => handleUpdateStatus(sub.submission_id, "submitted")}
                    ...
                    {sub.status === "submitted" && (
                      <button
                        onClick={() => handleUpdateStatus(sub.submission_id, "under_review")}
                    ...
                    {sub.status === "under_review" && (
                      <>
                        <button
                          onClick={() => handleUpdateStatus(sub.submission_id, "approved")}
                    ...
                    {sub.status === "approved" && (
                      <button
                        onClick={() => handleUpdateStatus(sub.submission_id, "paid")}
      ...
      {navTab === "inspector" && selectedSub && (
        <div className="flex flex-col gap-5 text-left font-mono text-xs">
          <div className="p-6 bg-slate-900/80 border border-white/10 rounded-2xl flex justify-between items-center">
            <div>
              <span className="text-[10px] text-sky-400 uppercase font-bold">Inspect Application Payload</span>
              <h3 className="text-lg font-bold text-white mt-0.5">{selectedSub.submission_id}</h3>
              <p className="text-xs text-slate-400">Homeowner: {selectedSub.homeowner?.name} | Reference: {selectedSub.seaiReference}</p>
            </div>
            <StatusBadge status={selectedSub.status} />
          </div>

          <div className="p-5 bg-slate-950/80 border border-white/10 rounded-2xl overflow-x-auto">
            <h4 className="text-xs font-bold text-slate-400 mb-2">Raw JSON Payload (GRANT_SUBMISSIONS KV Format)</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-5 bg-slate-900/80 border border-white/10 rounded-2xl flex flex-col gap-3">
              ...
              <div className="flex justify-between items-center p-3 bg-slate-950/80 rounded-xl border border-white/5">
                ...
                <span className="text-emerald-400 font-bold text-sm">{metrics.approvalRate}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-slate-950/80 rounded-xl border border-white/5">
                <span className="text-slate-400">Payment Disbursement Rate:</span>
                <span className="text-yellow-400 font-bold text-sm">{metrics.paymentRate}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-slate-950/80 rounded-xl border border-white/5">
                <span className="text-slate-400">Average Approval Duration:</span>
                <span className="text-indigo-300 font-bold text-sm">{metrics.averageApprovalDays}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-slate-950/80 rounded-xl border border-white/5">
                <span className="text-slate-400">Average Payment Duration:</span>
                <span className="text-purple-300 font-bold text-sm">{metrics.averagePaymentDays}</span>
              </div>
            </div>
        ...
        <div className="p-6 bg-slate-900/80 border border-white/10 rounded-2xl flex flex-col gap-4 font-mono text-xs text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
            {metrics.funnel.map((stage, idx) => (
              <div key={idx} className="p-3 bg-slate-950/80 border border-white/5 rounded-xl flex flex-col justify-between text-center">
                <span className="text-[10px] text-slate-400 uppercase font-bold">{stage.stage}</span>
                <span className="text-xl font-bold text-emerald-400 my-2">{stage.count}</span>
                <span className="text-[10px] text-sky-300">Avg: {stage.avgDuration}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {showRejectModal && selectedSub && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="glass-card p-6 border border-rose-500/30 rounded-2xl bg-slate-900 w-full max-w-md flex flex-col gap-4 font-mono text-xs text-left">
            <h3 className="text-sm font-bold text-rose-400">Reject Application ({selectedSub.submission_id})</h3>
            <p className="text-slate-300">Specify SEAI rejection reason to append to the status history audit trail:</p>
            <input
              type="text"
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => handleUpdateStatus(selectedSub.submission_id, "rejected", rejectionReasonInput || "SEAI Documentation Discrepancy")}
