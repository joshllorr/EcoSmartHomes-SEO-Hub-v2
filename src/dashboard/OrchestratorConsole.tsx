...
import { Cpu, Play, RefreshCw, CheckCircle2, ShieldCheck, Zap, Activity, Clock, Layers, Sparkles } from "lucide-react";
    ...
    <div className="flex flex-col gap-5 text-left font-sans">
      ...
      <div className="glass-card p-6 border border-emerald-500/20 rounded-2xl bg-slate-900/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold tracking-wider">Phase 40 Master Orchestrator</span>
          </div>
          <h2 className="text-xl font-bold text-white mt-1">EcoSmartHomes Unified AI Control Layer</h2>
          <p className="text-xs text-slate-400 font-mono mt-0.5">30-minute Cloudflare Edge heartbeat coordinating sentiment, coaching, scoring, insights, & forecasting.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-xs">
        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-emerald-400">
            <Clock size={18} />
            <span className="font-bold text-slate-300">Last Execution Timestamp</span>
          </div>
          <span className="text-xl font-bold text-emerald-400 mt-3">{new Date(state.lastRun).toLocaleString()}</span>
          <span className="text-[10px] text-slate-400 mt-1">Next Heartbeat: ~30 mins</span>
        </div>

        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-sky-400">
            <Activity size={18} />
            <span className="font-bold text-slate-300">Total Autonomous Cycles</span>
          </div>
          <span className="text-3xl font-bold text-sky-300 mt-3">{state.cycles}</span>
          <span className="text-[10px] text-slate-400 mt-1">Continuous Edge Execution</span>
        </div>

        <div className="glass-card p-5 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-indigo-400">
            <Layers size={18} />
            <span className="font-bold text-slate-300">Managed KV Namespaces</span>
          </div>
          <span className="text-3xl font-bold text-indigo-300 mt-3">26 Namespaces</span>
          <span className="text-[10px] text-slate-400 mt-1">100% Edge Bound & Audited</span>
        </div>
      </div>
      ...
      <div className="glass-card p-6 border border-white/10 rounded-2xl bg-slate-900/60 flex flex-col gap-4 font-mono text-xs">
        <div className="flex justify-between items-center">
          ...
          <span className="text-[10px] text-slate-400">{state.lastActions.length} Actions Recorded</span>
        </div>
