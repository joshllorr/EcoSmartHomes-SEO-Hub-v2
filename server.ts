...
// Helper functions for maps grounding and supplier classification
function classifySupplierQueryServer(prompt: string): string {
...
function getLocalSuppliersDatasetServer(
  category: string,
  lat = 52.6638,
  lng = -8.6267,
) {
...
const handleMapsGroundingRoute = async (req: any, res: any) => {
  const prompt =
    req.body?.prompt ||
    req.query?.prompt ||
  ...
  const latitude =
    parseFloat(
      String(req.body?.latitude ?? req.query?.latitude ?? '52.6638'),
    ) || 52.6638;
  const longitude =
    parseFloat(
      String(req.body?.longitude ?? req.query?.longitude ?? '-8.6267'),
    ) || -8.6267;

  const category = classifySupplierQueryServer(prompt);
  const dataset = getLocalSuppliersDatasetServer(category, latitude, longitude);
...
Always recommend registered contractors, sustainable materials, and accurate details of options available in Ireland (like SEAI grants, BER ratings, heat pump installers, insulation types).`,
    ...
    const text = response.text || dataset.text;
    ...
    return res.json({
      success: true,
      text,
      sources: combinedSources.length > 0 ? combinedSources : dataset.sources,
      isMock: false,
      provider: 'EcoSmart Maps Grounding Engine (Gemini 2.5 Live)',
    });
  } catch (error: any) {
    console.warn(
      'Gemini Maps Grounding warning, using verified dataset:',
      error?.message || error,
    );
    return res.json({
      success: true,
...
app.post('/api/energy/maps-grounding', handleMapsGroundingRoute);
app.get('/api/energy/maps-grounding', handleMapsGroundingRoute);
